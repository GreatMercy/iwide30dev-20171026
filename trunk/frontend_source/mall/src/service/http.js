 ...
 */
const getHotelList = (data, config, version = 'v1') => {
  let url = apiConfig[version] && apiConfig[version].GET_HOTEL_LIST || apiConfig.v1.GET_HOTEL_LIST
  return ajax.get(url, data, config)
}
/**
 * 微信订房 - 价格日历 - 信息
 ...
 * @param {String} data.rmid 房间ID
 */
const getHotelInfo = (data, config, version = 'v1') => {
  let url = apiConfig[version] && apiConfig[version].GET_HOTEL_INFO || apiConfig.v1.GET_HOTEL_INFO
  return ajax.get(url, data, config)
}
/**
 * 微信订房 - 价格日历 - 日历可预订时间
