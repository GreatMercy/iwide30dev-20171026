    ...
    created () {
      ...
      */
      ...
      if (!finish && now < end) {
        ...
        that.killsecTime = new KillsecTime({
          ...
          callback: function (type, val, ctx) {
            ...
            if (ctx.process === 0) {
              ...
              ctx.close()
              that.stopCycleGetKillsecStock()
              this.$emit('killsec-status', 4)
            } else if (ctx.process === 2) {
              // 已开始
              // that.stopCycleGetKillsecStock()
              if (!hasEmitStart) {
                that.$emit('killsec-status', 3)
                hasEmitStart = true
                that.interval = setInterval(function () {
                  that.getKillsecStockInfo()
                }, stock_reflesh_rate)
              }
            } else if (ctx.process === 1) {
              that.$emit('killsec-status', val < advanceTenMinutes ? 1 : 2)
            }
        ...
        that.$emit('killsec-status', finish ? 5 : 4)
        that.visible = false
      }
    },
    props: {
