<template>
  ...
  <section class="padding_0_15 bg_show">
    ...
    <p class="padding_top_78 font_14 color1 mar_l40">{{notice}}</p>
    <p class="font_14 color3 mar_t40">{{dataList.filed_name.balance_name}}充值</p>
    <div>
      <div class="condition h30 color1 mar_t40 txt_l">
        <div v-for="(value,key) in dataList.deposit_list" @click="chooseMoney(key,value.notice)" :class="{active:item === key}"><p><em class="font_15 margin_right_2 pclight">¥</em><em class="font_21 pclight">{{value.money * 1}}</em></p><span class="iconfonts main_color1 h48">&#xE031;</span></div>
      </div>
    </div>
    <div class="flex form_item buydeposit-input bd_top bd_bottom margin_top_20 white_bg">
      <div class="margin_right_23 width_130"><div class="flex between">
        ...
        <div class="flex_1 font_14"><input type="number" class="color1 font_16" v-model="inputmoney" name="money" @click="item='',notice='',sendType=1" placeholder="输入其他金额(充值上限10000元)"></div>
    </div>
  </section>
...
export default {
  ...
  created () {
    getBuydepositInfo().then((res) => {
      ...
      if (res.web_data.page_resource && res.web_data.page_resource.links.redirect) {
        window.location.replace(res.web_data.page_resource.links.redirect)
      }
      this.notice = this.dataList.deposit_list[0].notice
    })
  },
  methods: {
    submit () {
