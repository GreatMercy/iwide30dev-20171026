  ...
  <div class="jfk-pages jfk-page__orderCoupon">
    ...
    <div class="orderCoupon jfk-pl-30 jfk-pr-30">
      <div class="orderCoupon__main">
        ...
        <div class="jfk-menuList" v-if="validCoupon&&used&&overdue&&refunded">
            <div class="menu-inn jfk-flex">
                <div class="item" v-if="menuList.canReserve"><a :href="pageResource.package_booking+'&asset_item_id='+validCoupon.asset_item_id"><i class="jfk-font icon-user_icon_Checkin_normal"></i><p class="font-size--24">预约</p></a></div>
                <div class="item" v-if="menuList.canCheck"><a :href="pageResource.package_usage+'&asset_item_id='+validCoupon.asset_item_id"><i class="jfk-font icon-mall_icon_orderDetail_verify"></i><p class="font-size--24">验券</p></a></div>
                <div class="item" v-if="menuList.canPost"><a :href="pageResource.show_shipping_info+'&order_id='+validCoupon.order_id"><i class="jfk-font icon-mall_icon_orderDetail_post"></i><p class="font-size--24">邮寄</p></a></div>
                <div class="item" v-if="menuList.canSend"><a :href="pageResource.package_send+'&product_id='+productPackage.product_id"><i class="jfk-font icon-user_icon_Polite_nor"></i><p class="font-size--24">转赠</p></a></div>
            ...
            <div class="coupon__detail__list" >
              <template v-for="item ,idx in coupons" >
                <div class="item jfk-flex"  :class="{ item__invalid : item.status=='3'&&item.shipping_id=='0',item__post: item.status=='3'&&item.shipping_id!='0',item__send: item.status=='4'}"  v-if='idx==0' >
                    ...
                    <div class="item__right" v-if="item.status=='2'" @click="handleQrcode(item.code)">
                      <a href="javascript:;">
                    ...
                    <div class="item__right" v-else-if="item.status=='3'&&item.shipping_id!='0'">
                      <a :href="pageResource.shipping_detail+'&shipping_id='+item.shipping_id">
                        <span class="coupon__state font-size--24">已邮寄</span>
                        <i class="jfk-font icon-user_icon_jump_normal arrow"></i>
                      </a>
                    </div>    
                    <div class="item__right" v-else-if="item.status=='3'&&item.shipping_id=='0'">
                      <a :href="pageResource.package_review+'&consumer_item_id='+item.consumer_item_id">
                ...
                <div class="item jfk-flex"  :class="{ item__invalid : item.status=='3'&&item.shipping_id=='0',item__post: item.status=='3'&&item.shipping_id!='0',item__send: item.status=='4'}"  v-else-if='couponsShow' >
                    ...
                    <div class="item__right" v-if="item.status=='2'" @click="handleQrcode">
                      <a href="javascript:;">
                        <i class="jfk-font icon-mall_icon_pay_focus qrcode"></i>
                        <i class="jfk-font icon-user_icon_jump_normal arrow"></i>
                      </a>
    ...
    <div class="orderBtns jfk-pl-30 jfk-pr-30">
        <div class="jfk-flex btn-box">
          <div class="btn-item" v-if="canRefundOrder">
            <a :href="pageResource.refund_index">
          ...
          <div class="btn-item" v-if="canDeleteOrder" @click="handleDeleteOrder(pageResource.del_order,product.order_id)">
            <a >
    ...
    <jfk-popup v-model="showQrcode" :showCloseButton="true" class="coupon-qrcode jfk-ta-c">
     <qriously :value="popQrcodeNum" :size="200" ></qriously>
       ...
       <div class="tradeBox">
        ...
        <div class="order__gift__info jfk-clearfix">
         ...
         <div class="trade-product font-size--24 jfk-ta-c">
           <a herf="">查看商品详情<i class="jfk-font icon-home_icon_Jump_norma"></i></a>
         </div>       
    </jfk-popup>    
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleDeleteOrder (link, orderId) {
        this.$jfkConfirm('确定删除订单？').then(action => {
          if (action === 'confirm') {
            window.location.href = link + '&order_id=' + orderId
          }
        }).catch(() => {
        })
      },
