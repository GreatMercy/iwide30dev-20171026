<template>
  <div class="jfk-pages jfk-pages__cityChoose">
    ...
    <p class="float_word font-size--22" v-show="searchStatus">
    ...
    <div class="search_city font-size--38 jfk-pl-30 jfk-pr-30">
      <form @submit.prevent="searchHotelAction()">
        <input type="text" placeholder="关键字 / 位置 / 名称" class="font-size--38" autocomplete="off" v-model="searchInputVal"
               id="searchVal">
    ...
    <div class="search_city font-size--32 jfk-pl-30 jfk-pr-30" v-show="searchStatus"
         @click="toLocationHref(toLinks.NEARBY)">
        <span class="no_far">
    ...
    <div class="history_search font-size--30 jfk-pl-30 jfk-pr-30" v-show="searchStatus">
      ...
      <span class="hot_item" v-for="(item, index) in hotCitys" :key="index"
            @click="toLocationHref(toLinks.SRESULT+'&keyword='+item)">{{item}}</span>
    </div>
    <!--所有城市-->
    <div class="all_city font-size--28 jfk-pl-30 jfk-pr-30" v-show="searchStatus">
      <div class="all_city_item"
           v-for="(item,value) in allCitys">
        <p class="all_city_item_word">{{value}}</p>
        <ul v-for="(item1, index1) in item"
            :key="index1">
          <li @click="toLocationHref(toLinks.SRESULT+'&city_val='+item)">{{item1.city}}</li>
        </ul>
      </div>
    </div>
    <!-- 异步获取酒店列表 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchHotelAction () {
        let postData = {
          start: this.handleStartDate,
          end: this.handleEndDate,
