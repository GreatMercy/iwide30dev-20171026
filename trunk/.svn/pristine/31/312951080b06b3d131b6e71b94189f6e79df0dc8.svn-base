    ...
    <?php if(!empty($pay_ways)) foreach($pay_ways as $k=>$pw){?>
        <div pay_type='<?php echo $pw->pay_type;?>' class="pay_way <?php if($k==0){?>ischeck<?php }?>">
			...
			<span><?php echo $pw->pay_name;?><?php if($pw->pay_type=='balance') { ?>(<?php echo $member->balance;?>元)<?php }
		        else if($pw->pay_type=='point') {?>(<?php if(!empty($pw->des)) echo $pw->des; elseif($inter_id=='a421641095')echo $member->bonus.'/'.$member->bonus/10;else echo $member->bonus.'/'.$member->bonus/100;?>)<?php }?></span>
        </div>
        <?php }?>
...
<div style="padding-top:15%">
    <div class="foot_fixed">
    	<div class="color_minor pad3 bd_top_img">
        	...
        	<span id="total_price" class=" h36"><?php echo $first_state['total_point'];?>积分</span>
        	<?php }else{?>
