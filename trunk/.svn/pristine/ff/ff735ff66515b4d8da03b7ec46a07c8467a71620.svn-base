      ...
      <div class="row">
        ...
        <div class="col-xs-12">
		  ...
		  <?php if (empty($no_link_saler)){?>
                  ...
                  <div class="radio">
                    <label><input type="radio" name="beyond" <?php if(isset($row['id']) && $row['beyond']==4) echo 'checked';?> value="4">泛分销</label>
                    <div class="showchild">
                      <p>价格代码</p>
                        <ul>
                          <li>
                            <label><input type="checkbox" class="checkall"> 全选</label>
                            <label><input type="checkbox" name="link_f_all" <?php if(isset($row['id']) && $row['beyond']==4 && $row['all']==1) echo 'checked';?>> 不限制价格代码</label>
                          </li>
                          <?php foreach ($price_code as $value) {
                            echo "<li><label><input type='checkbox' name='link_f_pc' value='$value[price_code]'";
                            if(isset($row['id']) && $row['beyond']==4 && in_array($value['price_code'],$row['price_typeid'])){
                              echo 'checked';
                            }
                            echo ">$value[price_name]</label></li>";
                          }
                      ...
                      <p>支付方式</p>
                        <ul>
                          <li><label><input type="checkbox" class="checkall"> 全选</label></li>
                          <?php foreach ($pay_ways as $value) {
                            echo "<li><label><input type='checkbox' name='link_f_pw' value='$value->pay_type'";
                            if(isset($row['id']) && $row['beyond']==4 && in_array($value->pay_type,$row['pay_wayid'])){
                              echo 'checked';
                            }
                            echo ">$value->pay_name</label></li>";
                          }
...
function sub(){
  ...
  }else if(beyond == '4'){//泛分销
    $("input[name='link_f_pc']:checked").each(function(index){
        price_typeid[index] =$(this).val();
    });
    $("input[name='link_f_pw']:checked").each(function(index){
        pay_wayid[index] =$(this).val();
    });
    if($("input[name='link_f_all']").is(':checked')){
      var all = 1;
    }else{
      var all = 2;
    }
