			...
			<div class="item_foot"  <?php if(!$cur_label_count&&$i>0){?>style="display:none;"<?php }?>>
				...
				<div class="bd_bottom list_style" style="background:none">
					<?php if(!empty($r['state_info'])){ foreach($r['state_info'] as $si){?>
						...
						<div class="h24 color_main"><?php echo isset($si['bookpolicy_condition']['breakfast_nums'])?$si['bookpolicy_condition']['breakfast_nums']:'';?></div>
								...
								<?php if($si['avg_price']>=0){ ?>
									<div class="color_minor">?<?php echo $si['avg_price'];?>
                                        ...
                                        <?php if($si['wxpay_favour_sign']==1){?>
                                            <p class="h24 color_main" style="color:rgba(19, 176, 43, 0.83)"><?php echo '立减'.$si['bookpolicy_condition']['wxpay_favour'].'元';?></p>
                                        <?php }?>
									</div>
								<?php } ?>
								<?php if($si['book_status']=='available'){?>
			...
			$.each(data.rooms,function(i,n){
				...
				if(n.state_info!=''){
					...
					$.each(n.state_info,function(sk,sn){
						...
						if(sn.bookpolicy_condition!=undefined&&sn.bookpolicy_condition.breakfast_nums!=undefined){
							temp+=sn.bookpolicy_condition.breakfast_nums;
						}
						temp+='</div>';
                        ...
                        if(sn.point_exchange!=undefined){
							...
							if(parseFloat(sn.avg_price)>=0){
                                ...
                                if(sn.wxpay_favour_sign==1) temp+=' <p class="h24 color_main" style="color:rgba(19, 176, 43, 0.83)">立减'+sn.bookpolicy_condition.wxpay_favour + '元</p>';
								temp+='</div>';
							}
							if(sn.book_status=='available'){
								temp+='<span onclick="pay(this,event)" room_id="'+n.room_info.room_id+'"';
