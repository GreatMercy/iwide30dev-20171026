...
//*/
	...
	function search_staff($inter_id,$keyword) {
		...
		return $this->db->query($sql,array($inter_id,$keyword,"%$keyword%"))->result_array();
	}
	...
	function get_club_by_id($inter_id,$club_id,$format=TRUE){
		...
		if (!empty($result)&&$format){
			...
			$this->db->where(array('qrcode_id'=>$result['id'],'inter_id'=>$inter_id));
			$staff=$this->db->get('hotel_staff')->row_array();
