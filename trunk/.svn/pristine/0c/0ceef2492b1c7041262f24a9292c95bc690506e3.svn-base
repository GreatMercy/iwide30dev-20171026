...
<div class="gradient_bg wrapper">
    ...
    <?php }?>
    <div class="layer_bg border_radius mar_t80 pad_lr40 pad_b30 pad_t60">
...
function sub_comment(){
    ...
    if ($('.comment_publish_text').get(0).value.length<10){
        $.MsgBox.Alert("评论内容不得少于10个字符")
        return false;
    }
            ...
            function upload() {
                wx.uploadImage({
                    ...
                    success: function (res) {
                        ...
                        $("#add_publish_comment").before(_html)
                        i++;
