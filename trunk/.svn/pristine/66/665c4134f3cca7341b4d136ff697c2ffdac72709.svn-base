...
class Orders extends MY_Admin {
    ...
    public function edit()
    {
        ...
        $html= $this->_render_content($this->_load_view_file($view), $view_params, TRUE);
        //echo $html;die;
        echo $html;
    }
					...
					if($r){
						...
						if(2 == $result['pay_way']){//余额支付 stgc 20160912
                            ...
                            if(is_array($refund_result) && "SUCCESS" == $refund_result['return_code'])
                            {
                                ...
                                echo json_encode ( array (
                                    'status' =>0,
                                    'message' => "向微信服务器发起退款的时候失败（{$refund_result['return_msg']}），请稍后再试"
                                ) );
                            }
	...
	public function ext_okpay_orders_report($filter = array(),$per_page = 30,$page=1){
		...
		$data = $data."\n";
		$paystatus = array(1=>'未支付',3=>'已支付',4=>'已退款');
