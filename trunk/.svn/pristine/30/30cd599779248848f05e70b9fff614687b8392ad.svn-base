...
class Distribute_notice_model extends MY_Model {
	...
	public function create_notice($params){
		...
		$datas ['push_time']   = isset ( $params ['push_time'] ) ? $params ['push_time'] : date ( 'Y-m-d H:i:s' );
		...
		log_message('error', 'CREATE_NOTICE | '.$this->_db('iwide_rw')->last_query().' | ' . json_encode($datas));
		...
		if (empty ( $params ['openid'] )) {
			...
			if(empty($params['qrcodes'])){
				if($datas ['hotel_id'] > 0){
					$sql .= ' AND hotel_id=?';
					$query_params[] = $datas ['hotel_id'];
				}
			}else{
				$sql .= is_array($params['qrcodes']) ? ' AND qrcode_id IN ?' : 'AND qrcode_id=?';
				$query_params[] = $params['qrcodes'];
			}
		} else {
		...
		if ($this->_db('iwide_rw')->trans_status () === FALSE) {
			...
			log_message('error', 'CREATE_NOTICE | '.$this->_db('iwide_rw')->last_query().' | ' . json_encode($query_params));
			return TRUE;
		}
	}
	public function update_notice($eid,$params){

	}
	public function add_package_log($params){
		return $this->_db('iwide_rw')->insert('distribute_package_log',$params);
	}
	public function get_package_log_count($eid,$pid){
		$this->_db('iwide_r1')->where(array('eid'=>$eid,'pid'=>$pid));
		$this->_db('iwide_r1')->select('id');
		return $this->_db('iwide_r1')->get('distribute_package_log')->num_rows();
	}
	/**
	 * 将消息标记为已读
