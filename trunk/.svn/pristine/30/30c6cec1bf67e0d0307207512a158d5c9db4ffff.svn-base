...
let getProductKillsecProps = function (pid, price, stock) {
  ...
  return {
    ...
    'end_time': killsecTime.add(Random.integer(1, 3), 'm').format(momentFormatTpl),
 ...
 */
let getProductBaseTpl = function (type = 1, shouldWrapProductId = false) {
  ...
  let product = {
    ...
    'face_img': imgLists[parseInt(Math.random() * 9)]
  }

  if (type === 2) {
    product.killsec = getProductKillsecProps(productId, price, stock)
  }

  if (type === 3) {
    product.groupon = getProductGrouponProps(productId, price, stock)
  }

  if (shouldWrapProductId) {
    return {
      [productId]: product
    }
  } else {
