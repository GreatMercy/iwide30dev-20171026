...
class Dis_ext extends MY_Admin{
     ...
     * 导出报表
     */
    protected function ext_grades($data)
    {
        $res = $data['res'];
        ...
        $objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 10, 1, '备注' );
        ...
        foreach ( $res as $item ) {
            ...
            $objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 9, $row, empty($sstatus[$item['status']]) ? '-' : $sstatus[$item['status']] );
            $objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 10, $row, empty($item['remark']) ? '-' : $item['remark'] );
            $row ++;
        }
