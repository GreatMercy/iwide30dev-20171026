  ...
  <div class="jfk-pages jfk-pages__detail">
    ...
    <div v-if="productInfo.product_id">
      <div class="hotel-staff jfk-pl-30 jfk-pr-30" v-if="salerBanner.length">
        <div class="cont color-golden font-size--24"><span class="jfk-font notice-icon font-size--24 icon-mall_icon_notice"></span><span class="jfk-font notice-icon-1 font-size--24 icon-mall_icon_1_notice"></span>{{salerBanner[0]}}<span v-if="salerBanner[1]" class="number jfk-font-number font-size--48">{{salerBanner[1]}}</span><span v-if="salerBanner[2]" class="unit">{{salerBanner[2]}}</span>{{salerBanner[3]}}</div>
      </div>
      ...
      <div class="info">
        ...
        <div class="sales font-color-light-gray font-size--24">
          ...
          <span class="sales_num">已售<i class="number">{{productInfo.sales_cnt}}</i></span>
        </div>
      ...
      <div v-if="productInfo.tag === 2 && !killsecButtonDisabled" class="killsec-original jfk-ml-30 jfk-mr-30">
        <div class="killsec-original__cont jfk-flex is-align-middle is-justify-space-between" @click="handleSubmitOrderOriginal">
          ...
          <span class="font-size--28 color-golden"><i>{{priceMarket}}</i><i class="jfk-font triangle font-size--24 font-color-extra-light-gray icon-user_icon_jump_normal"></i></span>
        </div>
      </div>
      <div class="graphic jfk-pl-30 jfk-pr-30" v-once v-if="productInfo.img_detail">
        <div class="title font-color-white font-size--32"><span>图文详情</span></div>
      ...
      <div class="notice jfk-pl-30 jfk-pr-30" v-once v-if="productInfo.order_notice">
        <div class="title font-color-white font-size--32"><span>订购须知</span></div>
      ...
      <div class="detail jfk-pl-30 jfk-pr-30 font-size--28" v-once v-if="productDetail">
        <div class="title font-color-white font-size--32"><span>商品内容</span></div>
        <ul class="cont">
          <li class="item thead font-color-light-gray">
            商品名称
            <i class="right">数量</i>
          </li>
          <li class="item tr font-color-extra-light-gray" v-for="item in productDetail">
            {{item.content}}
    ...
    <jfk-popup class="jfk-popup__qrcode jfk-ta-c" :showCloseButton="true" :closeOnClickModal="false" v-model="qrcodeVisible" v-if="hotelInfo.qrcode">
      ...
      <div class="tip font-size--28 font-color-extra-light-gray"><span>享受</span><span class="color-golden font-size--34"><i class="jfk-font icon-font_zh_geng_qkbys"></i><i class="jfk-font icon-font_zh_duo_qkbys"></i><i class="jfk-font icon-font_zh_you__qkbys"></i><i class="jfk-font icon-font_zh_hui_qkbys"></i></span></div>
    </jfk-popup>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitOrder (killsec) {
        location.href = this.payUrl + (this.settingId !== '-1' ? '&psp_id=' + this.settingId : '') + (killsec ? '&token=' + this.tokenId : '')
      },
      handleSpecTicket () {
