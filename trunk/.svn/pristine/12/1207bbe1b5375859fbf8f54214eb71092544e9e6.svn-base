...
// 		$sql = 'SELECT f.openid,f.nickname,f.headimgurl,f.inter_id,fl.event_time subcribe_time FROM (SELECT * FROM '.$this->_db('iwide_r1')->dbprefix('fans').' WHERE inter_id=?) f RIGHT JOIN (SELECT openid,event_time,inter_id FROM (SELECT * FROM '.$this->_db('iwide_r1')->dbprefix('fans_sub_log').' WHERE `event`=2 AND inter_id=? GROUP BY openid) ds WHERE `source`=? GROUP BY openid ORDER BY event_time DESC) fl ON f.openid=fl.openid AND f.inter_id=fl.inter_id';
// 		$sql = 'SELECT f.openid,f.nickname,f.headimgurl,f.inter_id,fl.event_time subcribe_time FROM (SELECT * FROM '.$this->_db('iwide_r1')->dbprefix('fans').' WHERE inter_id=?) f RIGHT JOIN (SELECT openid,event_time,inter_id FROM '.$this->_db('iwide_r1')->dbprefix('fans_sub_log').' WHERE id IN (SELECT min(id) FROM '.$this->_db('iwide_r1')->dbprefix('fans_sub_log').' WHERE `event`=2 AND inter_id=? GROUP BY openid) and `source`=?) fl ON f.openid=fl.openid AND f.inter_id=fl.inter_id';
...
// 								LEFT JOIN (SELECT `nickname`,`headimgurl`,`openid`,`inter_id` FROM iwide_fans WHERE inter_id=?) f ON f.openid=sd.openid AND f.inter_id=sd.inter_id limit 1";
        $sql = "SELECT hs.cellphone, hs.hotel_name, hs.hotel_id, hs.inter_id, hs.`name`, hs.qrcode_id saler_id, hs.openid, f.nickname, f.headimgurl FROM iwide_hotel_staff AS hs LEFT JOIN iwide_fans AS f ON( f.openid   = hs.openid AND f.inter_id=hs.inter_id ) WHERE hs.inter_id=? AND hs.openid=? limit 1";
...
// 		var_dump($this->input->post());exit;
    ...
    function save_staff_to_saler($inter_id,$saler){
        ...
        if($count_query['nums'] < 1){
            ...
            return $this->_db()->query($sql,array($saler,$inter_id)) > 0;
...
// 			FROM iwide_distribute_grade_all dga WHERE inter_id=? AND saler>0';
// 		if($type == 'DAY'){
// 			$sql .= " AND DATE_FORMAT(dga.grade_time,'%Y-%m-%d')=DATE_FORMAT(NOW(),'%Y-%m-%d')";
// 		}elseif($type == 'MONTH'){
// 			$sql .= " AND DATE_FORMAT(dga.grade_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')";
...
// 		array_push($param,$inter_id);print $sql;die;
    ...
    function get_fans_ranking($inter_id,$type='MONTH',$limit=15){
		...
		(SELECT *,@ranking:= @ranking + 1 rank  FROM (SELECT dsa.*,hs.`name`,hs.business,hs.master_dept,hs.hotel_id,hs.cellphone,hs.hotel_name,hs.`status`,hs.is_distributed,hs.openid
			...
			WHERE inter_id=? AND grade_table='iwide_fans_sub_log' AND saler>0";
        if($type == 'DAY'){
...
// 		return $this->db->query($sql,array($inter_id,$inter_id,$saler,$inter_id))->row_array();
    ...
    function get_single_user_ranking($inter_id,$type='MONTH',$saler){
        $sql = 'SELECT sl.*, f.nickname, f.headimgurl, f.openid FROM (SELECT s.*, hs.hotel_id, hs.`name`, hs.`cellphone`, hs.openid, hs.hotel_name FROM (SELECT a.`saler`, a.total_amount, a.inter_id, a.order_amount, a.grade_amount,  a.nums, @ranking:= @ranking + 1 rank FROM (SELECT SUM(grade_total) total_amount, COUNT(*) nums, dga.saler, dga.inter_id, dga.order_amount, dga.grade_amount FROM iwide_distribute_grade_all dga WHERE inter_id=? AND saler >0';
        if($type == 'DAY'){
            $sql .= " AND DATE_FORMAT(dga.grade_time,'%Y-%m-%d')=DATE_FORMAT(NOW(),'%Y-%m-%d')";
        }elseif($type == 'MONTH'){
            $sql .= " AND DATE_FORMAT(dga.grade_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')";
        ...
        $sql .= ' GROUP BY dga.saler ) a, (SELECT @ranking := 0 ) b ORDER BY a.`total_amount` DESC ) s INNER JOIN iwide_hotel_staff AS hs ON s.saler = hs.qrcode_id AND hs.inter_id =s.inter_id WHERE s.inter_id=? AND hs.is_distributed=1 AND hs.`status` =2 ORDER BY s.rank ASC ) sl LEFT JOIN iwide_fans AS f ON (sl.inter_id=f.inter_id AND sl.openid =f.openid) WHERE saler=? AND f.inter_id=?';
        $param = array($inter_id,$inter_id,$saler,$inter_id);
...
LEFT JOIN iwide_fans f ON f.openid=dg.grade_openid AND f.inter_id=dg.inter_id ORDER BY grade_time DESC";
    ...
    function i_get_user_ranking($inter_id,$type='MONTH',$limit=15){
        ...
        if(empty($obj)){
            ...
            $sql = 'SELECT sl.*, f.nickname, f.headimgurl, f.openid, @ranking:= @ranking + 1 rank FROM (SELECT s.*, hs.hotel_id, hs.`name`, hs.openid, hs.`cellphone`, hs.hotel_name FROM (SELECT a.`saler`, a.total_amount, a.inter_id, a.order_amount, a.grade_amount, a.nums FROM (SELECT SUM(grade_total) total_amount, COUNT(*) nums, dga.saler, dga.inter_id, dga.order_amount, dga.grade_amount, (SELECT @ranking := 0 ) b FROM iwide_distribute_grade_all dga WHERE inter_id=? AND saler >0';
            if($type == 'DAY'){
                $sql .= " AND DATE_FORMAT(dga.grade_time,'%Y-%m-%d')=DATE_FORMAT(NOW(),'%Y-%m-%d')";
            }elseif($type == 'MONTH'){
                $sql .= " AND DATE_FORMAT(dga.grade_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')";
            ...
            $sql .= ' GROUP BY dga.saler ORDER BY total_amount DESC LIMIT ? ) a ) s INNER JOIN iwide_hotel_staff AS hs ON s.saler =hs.qrcode_id AND hs.inter_id =s.inter_id WHERE s.inter_id=? AND hs.is_distributed=1 AND hs.`status` =2 ORDER BY s.total_amount DESC LIMIT ? ) sl LEFT JOIN iwide_fans AS f ON sl.inter_id=f.inter_id AND sl.openid =f.openid';
            $param = array($inter_id,$limit,$inter_id,$limit);
    ...
    function i_get_fans_ranking($inter_id,$type='MONTH',$limit=15){
        ...
        if(empty($obj)){
            ...
            $sql = "SELECT a.*,f.nickname,f.headimgurl,@ranking:= @ranking + 1 rank FROM
		(SELECT *  FROM (SELECT dsa.*,hs.`name`,hs.business,hs.master_dept,hs.hotel_id,hs.cellphone,hs.hotel_name,hs.`status`,hs.is_distributed,hs.openid
			FROM (SELECT inter_id,saler,grade_time,count(*) fans_count FROM iwide_distribute_grade_all
            ...
            $sql .= ") t ) a
			INNER JOIN iwide_fans f ON f.inter_id=a.inter_id AND a.openid=f.openid ORDER BY rank ASC";
            $obj = $this->_db('iwide_r1')->query($sql,$param)->result_array();
    ...
    function i_get_user_rank($inter_id,$type='MONTH',$saler){
        ...
        if(empty($obj)){
			...
			INNER JOIN (SELECT *,(SELECT @ranking := 0) b FROM iwide_hotel_staff WHERE inter_id=? AND is_distributed=1 AND `status`=2) hs
			ON hs.inter_id=dsa.inter_id AND hs.qrcode_id=dsa.saler ORDER BY fans_count DESC) t ) a
			INNER JOIN iwide_fans f ON f.inter_id=a.inter_id AND a.openid=f.openid AND a.saler=? limit 1";
