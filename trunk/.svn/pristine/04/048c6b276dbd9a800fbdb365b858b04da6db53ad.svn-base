...
class Hotel_cache_model extends CI_Model {
	...
	function get_cache_data($inter_id, $cache_type) {
		...
		if (is_array ( $cache_type )) {
			$this->db->where_in ( 'cache_type', $cache_type );
			$cache = $this->db->get ( self::TAB_CACHE )->result_array ();
			foreach ( $cache as $d ) {
				$arr [$d ['cache_type']] [$d ['ident']] = $d;
			}
		} else {
			$this->db->where ( 'cache_type', $cache_type );
			$cache = $this->db->get ( self::TAB_CACHE )->result_array ();
			foreach ( $cache as $d ) {
				$arr [$d ['ident']] = $d;
			}
	...
	function get_cache($inter_id, $type, $para = array()) {
		...
		$update_set = $this->Enum_model->get_enum_des ( 'HOTEL_CACHE_UPDATE_INTERVAL', 1 );
		if (is_array ( $type )) {
	...
	function get_ident_cache($inter_id,$ident,$cache_type) {
		...
		return $this->db->get ( 'hotel_cache' )->row_array ();
	}
}