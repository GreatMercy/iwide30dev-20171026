                ...
                <div class="box-body">
                	<?php echo form_open('distribute/welfare/save_config',array('id'=>'setting_form','class'=>'form-inline'));?>
						...
						<div class="form-group">
							...
							<option value="1"<?php if(isset($config->upper_limit) && $config->upper_limit == 1):echo ' selected';endif;?>>1次</option>
							<option value="2"<?php if(isset($config->upper_limit) && $config->upper_limit == 2):echo ' selected';endif;?>>2次</option>
							<option value="3"<?php if(isset($config->upper_limit) && $config->upper_limit == 3):echo ' selected';endif;?>>3次</option>
							<option value="4"<?php if(isset($config->upper_limit) && $config->upper_limit == 4):echo ' selected';endif;?>>4次</option>
							<option value="5"<?php if(isset($config->upper_limit) && $config->upper_limit == 5):echo ' selected';endif;?>>5次</option>
							</select>
						</div>
						<div class="form-group">
							<label>每日结算金额上限</label>&nbsp;<input class="form-control input-sm" type="text" id="upper_limit_day_amount" name="upper_limit_day_amount" value="<?php echo isset($config->upper_limit) ? $config->upper_limit : 200 ;?>" />
						</div>
						</div>
						</div>
						<div class="row">
