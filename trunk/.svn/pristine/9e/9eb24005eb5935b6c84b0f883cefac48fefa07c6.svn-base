...
class Orders extends MY_Admin {
	...
	public function index_one(){
		...
		$data['t_order_num'] = $model->get_today_order_num($this->inter_id);
		$data['t_checkin_num'] = $model->get_today_checkin_num($this->inter_id,$entity_id);
		$data['order_confirm_two'] = $this->getNumRoom(2);	
		$data['order_comfirm_num'] = $model->get_order_confirm_num($this->inter_id);
	...
	public function getNumRoom($num=1){
		...
		$data = $model->get_order_confirm_two($this->inter_id,$num);
		foreach ($data as $kt => $vt) {
			...
			$data[$kt]['days'] = round((strtotime($vt['enddate'])-strtotime($vt['startdate']))/86400);
	...
	public function grid() {
		...
		$data['allhotels'] = $model->get_all_hotels($this->inter_id);
		...
		$data ['search_url'] = site_url ( 'hotel_2/orders/index'  . "?s=" . $data ['istatus']);
		...
		$this->_render_content ( $this->_load_view_file ( 'new_index' ), $data, false );
	}
	...
	public function edit() {
		...
		if (! empty ( $order )) {
			...
			foreach ($order['order_details'] as $k => $item) {
				$arr = array();
				$countday = ceil ( (strtotime ( $item ['enddate'] ) - strtotime ( $item ['startdate'] )) / 86400 ); // 获得相差天数
	...
	public function item_edit_post() {
		...
		if (! empty ( $item_id ) && ! empty ( $orderid )) {
			$model->update_order_item ( $this->inter_id, $orderid, $item_id, $data );
		}
		redirect ( site_url ( 'hotel/orders/index' ) );
	}
		...
		for($i = 0; $i < $count;) {
			...
			for($j = 0; $j <= $result [$i]->roomnums; $j ++) {
				...
				if ($j == 0) {
					...
					$tmp ['enddate'] = date ( "Y-m-d", strtotime ( $result [$i + $j - 1]->ienddate ) );
					// $tmp ['roomnums'] = 1;
					$data [$result [$i]->orderid] ['total_room_nights'] += round ( (strtotime ( $result [$i + $j - 1]->ienddate ) - strtotime ( $result [$i + $j - 1]->istartdate )) / 86400 );
					if (in_array ( $result [$i + $j - 1]->istatus, $valid_status )) {
						$data [$result [$i]->orderid] ['real_room_nights'] += round ( (strtotime ( $result [$i + $j - 1]->ienddate ) - strtotime ( $result [$i + $j - 1]->istartdate )) / 86400 );
	...
	public function usertag() {
		$model = $this->_load_model ( $this->main_model_name () );
		$data=array();
		$this->_render_content ( $this->_load_view_file ( 'usertag' ), $data, false );
	}
    ...
    public function goods_list(){
        ...
        $data=array();
        $this->_render_content ( $this->_load_view_file ( 'goods_list' ), $data, false );
