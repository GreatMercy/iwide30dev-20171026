...
class Qrcode_model extends CI_Model{
	...
	function save_edit($inter_id=''){
		...
		$param['name']     = $this->input->post('hotel',TRUE);
		if(empty($key)){
			...
			$staff_param['hotel_id']       = $this->input->post('hotel_id',TRUE);
	...
	function get_qrcodes_list($inter_id,$nums = NULL,$offset = NULL){
		...
		return $this->db->get_where('qrcode',array('inter_id'=>$inter_id));
	}
	...
	function get_detail($id, $inter_id){
		$this->db->where(array('id'=>$id,'inter_id'=>$inter_id));
		$this->db->limit(1);
		return $this->db->get('qrcode');
	}
	...
	function scan_log($qrcode_id,$inter_id,$openid = NULL,$nickname = NULL,$event_time = NULL){
		...
		$sql = 'SELECT COUNT(*) nums FROM ' . $this->db->dbprefix ( "fans_sub_log" ) . ' WHERE `event`=2 AND inter_id=? AND openid=?';
		$count_query = $this->db->query($sql,array($inter_id,$openid))->row_array();
		...
		if (is_numeric ( $qrcode_id )) {
			...
			$sql.=" values (-1,'$event_time','','$inter_id')";
		}
