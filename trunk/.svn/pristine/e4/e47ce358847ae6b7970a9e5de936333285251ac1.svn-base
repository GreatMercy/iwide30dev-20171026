              ...
              <tr class="form_con">
                ...
                <td>
                  <div class="input_radios">
                      <input class="" type="radio" id="<?php echo $saler['qrcode_id'];?>"  <?php if($saler['qrcode_id']==$list['id'])echo 'checked'; ?> name="cat_img_12" value="<?php echo $saler['qrcode_id'];?>">
                      <label for="<?php echo $saler['qrcode_id'];?>">选择</label>
                  </div>
                </td>
              </tr>
            <?php }}?>
      ...
      <table id="coupons_table1" class="table-striped table-condensed dataTable no-footer color_555" style="width:100%;font-size:12px;">
        ...
        <?php if(!empty($hotels)){ foreach($hotels as $key=>$hotel){?>
              <tr class="form_con">
                ...
                <td>
                  <div class="input_checkboxs"><input type="checkbox" id="<?php echo $hotel['hotel_id'];?>" <?php if(in_array($hotel['hotel_id'],$array_hotel_ids))echo 'checked';?> value="<?php echo $hotel['hotel_id'];?>"><label for="<?php echo $hotel['hotel_id'];?>" style="width:auto;">选择</label></div>
                </td>
              </tr>
        <?php }}?>
        </tbody>
...
<div class="over_xs">
    <div class="content-wrapper">
        ...
        <form role="form" method='post' id='form1' action='<?php echo site_url('club_2/Club_list/save_club');?>'>
            <div class="">
                  ...
                  <div class="p_30_35 color_555">
                    ...
                    <div class="m_b_15">
                      ...
                      <div class="m_b_15">
                          ...
                          <span><input type="text" id="datepicker" name="start_range"  placeholder="<?php echo date("Y-m-d",strtotime($list['valid_times'][0]));?>" value="<?php echo date("Y-m-d",strtotime($list['valid_times'][0]));?>" /></span>-
                          <span><input type="text" id="datepicker2"  name="end_range"  placeholder="<?php echo date("Y-m-d",strtotime($list['valid_times'][1]));?>" value="<?php echo date("Y-m-d",strtotime($list['valid_times'][1]));?>" /></span>
                      </div>
                      <div class="m_b_15">
                ...
                <div>
                  ...
                  <input class="price_ids" type="hidden" name="price_code" value=""/>
                      <?php if(!empty($price_codes)){ foreach($price_codes as $key=>$code){ ?>
                        <div><input type="checkbox" <?php if(in_array($key,$list['price_code']))echo 'checked';?> id="<?php echo $key;?>"  value="<?php echo $key;?>" ><label for="<?php echo $key;?>"><?php echo $code['price_name'];?></label></div>
...
//     },'json')
// });
var hotel_arr=[<?php echo $list['hotel_id'];?>];
$('#coupons_table1 .input_checkboxs').on('click','input[type="checkbox"]',function(){
    ...
    console.log($('.rule_input').val());
})
function sub(){
	var room_arr=[];
	$('.rooms div input:checked').each(function(index, el) {
		room_arr.push($(this).val());
	});
	$('.price_ids').val(room_arr);
    $.post('<?php echo site_url('club_2/Club_list/save_club')?>',
        {
