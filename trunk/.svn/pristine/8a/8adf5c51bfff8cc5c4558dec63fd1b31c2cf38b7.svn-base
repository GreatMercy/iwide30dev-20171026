      ...
      <el-col :span="24">
        <div class="grid-content">
          ...
          <template v-if="activeStep === 0">
            <!--超级管理员-->
            <template v-if="inforData.admin_type === 1">
              <el-form :model="createdInfor" :rules="rules" ref="createdInfor1"
                       label-width="200px">
                <div>
            ...
            <template v-else-if="inforData.admin_type === 2">
              <el-form :model="createdInfor" :rules="rules" ref="createdInfor"
                       label-width="200px">
                <el-form-item label="选择公众号" prop="arr_inter_id">
                  <el-select placeholder="请选择公众号" v-model="createdInfor.arr_inter_id">
                    <el-option v-for="item in inforData.arr_inter_id" :key="item.inter_id" :lable="item.inter_id"
              ...
              <el-tabs v-model="activeTab" type="card">
                <el-tab-pane v-for="(item,index) in authorities" :key="index" :label="item.name" :name="'tab_'+ index">
                  ...
                  <div class="all-outer">
                    ...
                    <el-checkbox v-model="item.all_status">
                      默认开通全部（含未来新功能）
                    </el-checkbox>
                  </div>
                  <!--角色权限列表-->
                  <table class="table-outer">
                    ...
                    <tr v-for="(item1,index1) in item.controllers">
                      <td>
                        ...
                        <el-checkbox v-model="item1.all_status">全部</el-checkbox>
                      </td>
                      <td>
    ...
    methods: {
      ...
      getInforData () {
        getRoleInfor({role_id: this.role_id}).then((res) => {
          ...
          if (res.web_data.edit_role !== null && res.web_data.edit_role.length !== 0) {
            let itemTemp = this.inforData.arr_inter_id.find((item) => {
              return item.inter_id === res.web_data.edit_role.inter_id
            })
            this.createdInfor.status = res.web_data.edit_role.status
            this.createdInfor.arr_inter_id = itemTemp.inter_name
          }
          this.createdInfor.related_role_id = res.web_data.edit_role.related_role_id
          this.createdInfor.superRoleName = res.web_data.edit_role.role_name
          this.createdInfor.userDefinedName = res.web_data.edit_role.role_name
      ...
      // 获取权限列表
      getAuthorityData () {
        getRoleAuthority({role_id: this.role_id}).then((res) => {
          this.loading = false
          this.link = res.web_data.link
          this.authorities = res.web_data.role_authorities
          for (var i = 0; i < this.authorities.length; i++) {
            // 模块
            this.$set(this.authorities[i], 'all_status', this.authorities[i].all_private === 1)
            let controllersTemp = this.authorities[i].controllers
            for (var j = 0; j < controllersTemp.length; j++) {
              // 权限
              this.$set(this.authorities[i].controllers[j], 'all_status', controllersTemp[j].all_private === 1)
      ...
      // 点击完成
      postRoleData () {
        let interTemp = this.inforData.arr_inter_id.find((item) => {
          return item.inter_name === this.createdInfor.arr_inter_id
        })
        this.postData.related_role_id = this.createdInfor.related_role_id
        this.postData.inter_id = interTemp.inter_id
        this.postData.role_id = parseInt(this.role_id)
        ...
        this.postData.status = parseInt(this.createdInfor.status)
        for (var i = 0; i < this.authorities.length; i++) {
          // 插入模块
          // 默认全选
          if (this.authorities[i].all_status === true) {
          ...
          for (var j = 0; j < controllersTemp.length; j++) {
            // 权限
            // 默认全选
            if (this.authorities[i].controllers[j].all_status === true) {
              this.postData.controllers_all.push({name: 'controllers_all[]', value: controllersTemp[j].ctrl_id})
            }
