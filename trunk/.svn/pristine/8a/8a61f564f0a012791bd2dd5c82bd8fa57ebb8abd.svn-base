...
//        ];
    ...
    public function killsecBatch()
    {
        ...
        while($rows = $this->somaActivityKillsecGroupProductModel->search($params)) {
            ...
            $handler = new StreamHandler(APPPATH . 'logs/soma/killsec_group_cron/soma_' . date('Y-m-d') . '.log', \Monolog\Logger::DEBUG);
            $this->CI->monoLog->setHandlers(array($handler));
            $this->CI->monoLog->info('killsec_group_product count is ' . count($rows));
    ...
    public function monitorKillsecService()
    {
        ...
        $handler = new StreamHandler(APPPATH . 'logs/soma/killsec_monitor/soma_' . date('Y-m-d') . '.log', \Monolog\Logger::DEBUG);
        $this->CI->monoLog->setHandlers(array($handler));

        $this->CI->monoLog->info('check killsec sevice start!');
        ...
        $this->CI->monoLog->info('check killsec sevice end!');
    }
    ...
    protected function _checkRedisConnInfo($connData)
    {
        ...
        {
            $this->CI->monoLog->warning('killsec redis server disconnect!');
        }

        if($connData['l_server_conn'] == Soma_base::STATUS_FALSE)
        {
            $this->CI->monoLog->warning('cron lock redis server disconnect!');
        }
        else
        {
            if($connData['lock']['init_key'] == Soma_base::STATUS_FALSE)
            {
                // $this->CI->monoLog->warning('killsec initial cronjob is locked!');
            }
            if($connData['lock']['init_key'] == Soma_base::STATUS_FALSE)
            {
                // $this->CI->monoLog->warning('killsec order cronjob is locked!');
            }
        }
    }
    ...
    protected function _checkKillsecInstance($activitys, $instances)
    {
        ...
        if(count($nonExistIns) > 0)
        {
            ...
            $this->CI->monoLog->warning("The activity [$idStr] did not produce an instance in ten minutes!");
        }

        if(count($repeatIns) > 0)
        {
            $idStr = implode(',', $repeatIns);
            $this->CI->monoLog->warning("Activity [$idStr] instance is repeated!");
        }
    }
    ...
    protected function _checkKillsecRedis($instances, $redisData)
    {
        ...
        {
            $idStr = implode(',', $nonExistIns);
            $this->CI->monoLog->warning("The instance [$idStr] did not produce an redis data in ten minutes!");
        }

        if(count($lostCountIns) > 0)
        {
            $idStr = implode(',', $lostCountIns);
            $this->CI->monoLog->warning("instance [$idStr] redis killsec info seems losted!");
        }
    }
    ...
    public function monitorKillsecServiceLogSimple()
    {
        ...
        $this->CI->monoLog->warning('killsec initial cronjob is locked!');
        $this->CI->monoLog->warning('killsec order cronjob is locked!');
        $this->CI->monoLog->warning("The activity [1,2,3] did not produce an instance in ten minutes!");
        $this->CI->monoLog->warning("Activity [1,2,3] instance is repeated!");
        $this->CI->monoLog->warning("The instance [1,2,3] did not produce an redis data in ten minutes!");
        $this->CI->monoLog->warning("instance [1,2,3] redis killsec info seems losted!");
