...
class Fans_model extends MY_Model{
	 ...
	 * @return result row
	 */
	public function get_fans_info_by_id($inter_id,$fans_id,$f_key='id'){
		...
		return $this->_db('iwide_r1')->query ( $sql, array ( $fans_id, $inter_id ) )->row_array();
	}
...
// 					$params = array('saler' => $club_source,'openid'=>$saler_info['openid'],'nickname'=>$nickname);
