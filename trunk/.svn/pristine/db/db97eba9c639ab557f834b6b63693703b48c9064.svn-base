      ...
      refundReconfirmed (trimRefundVal) {
        ...
        .then((res) => {
          postrefund(qs.stringify(param), {headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then((res) => {
            let intTrimRefundVal = trimRefundVal * 100
            let intCanRefund = this.popItem.refund_amount * 100
            if (intTrimRefundVal < intCanRefund) {
              this.tableData[this.listIndex].refund_amount = (intCanRefund - intTrimRefundVal) / 100
              this.sendAjax = false
            } else {
