...
class Okpay_wxpay_model extends MY_Model{
	...
	function refund($inter_id,$transaction_id,$total_fee,$refund_fee,$our_refund_no){
        ...
        log_message("error","___2___return info::|obj::".serialize($refundOrder));
        log_message("error","___3___return info::|".$this->api_client_key_pem.'   ||'.$this->api_client_cert_pem.'   ||'.$this->api_mch_key);
		//商户根据实际情况设置相应的处理流程
    ...
    public function weifutong_refund($inter_id,$transaction_id,$total_fee,$refund_fee,$out_refund_no,$hotel_id){
       ...
       // $this->RequestHandler->setParameter('version','2.0');
        ...
        if($this->PayHttpClient->call()){
            ...
            log_message("error","weifutong__2__return info::|".$this->RequestHandler->getAllParameters('err_msg'));
            return false;
        }else{
