...
<section class="bg_fff martop center">
    ...
    <div class="pad3"  style="width:90%;margin:auto">
    	<div class="webkitbox pad3">
            ...
            <div class="bd_bottom h24" style="width:5rem;border-color:#555">共<?php if($order['price_type'] != 'athour'){?><?php echo round(strtotime($order['enddate'])-strtotime($order['startdate']))/86400;?>晚/<?php }?><?php echo $order['roomnums'];?>间</div>
...
<div style="padding-top:4rem">
    ...
    <?php if($can_cancel==1){?>
		...
		<?php }?>
		<?php if($re_pay==1){?>
		 <a href="<?php echo site_url('wxpay/hotel_order').'?id='.$inter_id.'&orderid='.$order['orderid'];?>" class="pad10  bg_main">支付订单</a>
		<?php }else{?>
