...
class Order_model extends MY_Model {
	...
	function get_rooms_change_calendar($rooms, $idents = array(), $condit = array()) {
		...
		$price_info_sql = "SELECT i.*,s.add_service_set sadd_service_set,s.must_date smust_date,s.status sstatus,s.external_code sexternal_code,s.external_way sexternal_way,s.related_cal_way srelated_cal_way,s.related_cal_value srelated_cal_value,s.room_id,s.hotel_id,s.price sprice,s.nums snums,s.use_condition suse_condition,s.coupon_condition scoupon_condition,s.bonus_condition sbonus_condition
						    FROM `" . $this->db->dbprefix ( self::TAB_HPI ) . "` i join 
							(SELECT * from `" . $this->db->dbprefix ( self::TAB_HPS ) . "` where room_id in ($room_ids) and inter_id = '" . $idents ['inter_id'] . "' and status in (1,2)) s
		...
		$price_info_set = $this->db->query ( $price_info_sql )->result_array ();
		$set = array ();
		$related_codes = array ();
		$my_coupons=NULL;
		$price_code_in = '';
...
// 				return $rooms;
		...
		$pset_sql = "SELECT * from `" . $this->db->dbprefix ( self::TAB_HPS ) . "` where $state_condition ";
		$state_sql = "SELECT state.*,state.price_code scode,pset.price_code,pset.room_id FROM ( $pset_sql ) pset 
					   left join (SELECT * from `" . $this->db->dbprefix ( self::TAB_HRS ) . "` where $state_condition and date >= " . $condit ['startdate'] . " and date <= $enddate) state 
		...
		if (! empty ( $related_codes )) {
			...
			$this->db->where ( "date >= " . $condit ['startdate'] . " and date <=" . $enddate, null, false );
			$related_states = $this->db->get ( self::TAB_HRS )->result_array ();
			...
			$related_code_set = $this->db->query ( $related_state_sql )->result_array ();
...
// 						$d ['state_info'] [$tmp_code]['extra_info']['protrol_code']=$cp['info'];
	...
	function get_rooms_change($rooms, $idents = array(), $condit = array()) {
		...
		$datas = array ();
		$price_info_sql = "SELECT i.*,s.add_service_set sadd_service_set,s.must_date smust_date,s.status sstatus,s.external_code sexternal_code,s.external_way sexternal_way,s.related_cal_way srelated_cal_way,s.related_cal_value srelated_cal_value,s.room_id,s.hotel_id,s.price sprice,s.nums snums,s.use_condition suse_condition,s.coupon_condition scoupon_condition,s.bonus_condition sbonus_condition,s.bookpolicy_condition sbookpolicy_condition FROM `" . $this->db->dbprefix ( self::TAB_HPI ) . "` i join (SELECT * from `" . $this->db->dbprefix ( self::TAB_HPS ) . "` where room_id in ($room_ids) and inter_id = '" . $idents ['inter_id'] . "' and status in (1,2)) s on s.inter_id=i.inter_id and s.price_code=i.price_code where i.status = 1 and i.channel_code='" . $channel_code . "' and i.inter_id='" . $idents ['inter_id'] . "' and i.type $type";
		if (! empty ( $price_codes )) {
			$price_codes = "'" . implode ( "','", explode ( ',', $price_codes ) ) . "'";
			$price_info_sql .= " and i.price_code in ( " . $price_codes . ')';
		} else if (! empty ( $extra_price_code )) {
...
// 						$d ['state_info'] [$tmp_code]['extra_info']['protrol_code']=$cp['info'];
	...
	function get_protrol_price_code($inter_id, $hotel_id, $protrol_code) {
		...
		$this->db->join ( self::TAB_HPS . ' s', 'i.inter_id=s.inter_id ' );
		$this->db->where ( array (
				's.hotel_id' => $hotel_id,
				'i.type' => 'protrol',
	...
	function create_order($inter_id, $orderdata, $datas, $subs, $room_nos = array()) {
		...
		if ($this->db->insert ( self::TAB_HO, $data )) {
			...
			$link = site_url ( 'hotel/hotel/orderdetail' ) . '?id=' . $inter_id . '&orderid=' . $data ['orderid'];
	...
	function get_order_details($inter_id, $idents = array(), $field = 'id') {
		...
		$sql = "select h.name hname,h.book_policy,h.intro_img,b.*,a.* from
		 (SELECT * FROM " . $this->db->dbprefix ( self::TAB_HO ) . " WHERE $s `$field` = '" . $idents ['id'] . "') a
			 join " . $this->db->dbprefix ( self::TAB_H ) . " h on a.inter_id=h.inter_id and a.hotel_id=h.hotel_id
		   join " . $this->db->dbprefix ( self::TAB_HOI ) . " b on a.orderid=b.orderid and a.inter_id=b.inter_id";
		return $this->db->query ( $sql )->result_array ();
	}
	...
	function get_order($inter_id, $idents = array()) {
		...
		$order_sql = "select oa.*,o.* from (
						select * from " . $this->db->dbprefix ( self::TAB_HO ) . " where $s $o inter_id='$inter_id' and isdel=0) o
		 				 join " . $this->db->dbprefix ( self::TAB_HOA ) . " oa on o.orderid=oa.orderid and o.inter_id = oa.inter_id";
		return $this->db->query ( $order_sql )->result_array ();
	}
...
// 			$order_condition .= " o.startdate<=" . date ( 'Ymd' ) . " and o.enddate>" . date ( 'Ymd' ) . " and ";
		...
		if (isset ( $idents ['timetype'] ) && is_numeric ( $idents ['timetype'] ) && $idents ['timetype'] > 0) {
			...
			if (!empty($idents ['start_t'])&&$start_time=strtotime($idents ['start_t'])){
...
// 					$order_condition .= " $timetype<='".date('Ymd',$end_time)."' and ";
	...
	function get_all_hotels($inter_id,$hotel_ids=''){
		...
		if(!empty($ids)){
			...
			$temp = trim($temp,',');
			if($temp){
				$sql = "SELECT hotel_id,inter_id,`name` FROM ".$this->db->dbprefix ( self::TAB_H )." WHERE hotel_id IN($temp) AND inter_id='$inter_id'";
				return $this->db->query($sql)->result_array();
			}else{
				return array();
			}
	...
	function get_today_order_num($inter_id,$entity_id=''){
		...
		$sql = "select count(*) as count from ".$this->db->dbprefix ( self::TAB_HO )." where inter_id='$inter_id' and order_time>='$t_time' and isdel=0 and $s status in(0,1,2,3,4,5,11)";
	...
	function get_today_checkin_num($inter_id,$entity_id=''){
		...
		$sql = "select count(*) as count from ".$this->db->dbprefix ( self::TAB_HO )." where inter_id='$inter_id' and startdate<='$t_date' and enddate>'$t_date' and isdel=0 and $s status in(0,1,2,3,4,5,11)";
		$count = $this->db->query($sql)->result_array();
		return $count[0]['count'];
	}
	...
	function get_order_confirm_two($inter_id,$num,$entity_id='',$order_fields=''){
		...
		foreach ($res as $kr => $vr) {
			$sqlc = "select count(1) count from ".$this->db->dbprefix ( self::TAB_HOI )." where inter_id='$inter_id' and orderid='".$vr['orderid']."' ";
			$count = $this->db->query($sqlc)->result_array();
			$res[$kr]['count'] = $count[0]['count'];
		}
		return $res;
	}
	...
	function get_order_confirm_num($inter_id,$entity_id=''){
		...
		$sql = "select count(*) count from ".$this->db->dbprefix ( self::TAB_HO )." where inter_id='$inter_id' and $s status=0 and isdel=0";
		$count = $this->db->query($sql)->result_array();
		return $count[0]['count'];
	}
		...
		if (! empty ( $check )) {
			...
			if ($state ['can_cancel'] == 1) {
				...
				if (! empty ( $check ['web_orderid'] )) {
					...
					$result = $this->pmsa->cancel_order ( $inter_id, $check );
...
// 											'istatus' => 0,
									...
									if($check_new_on>0){
										...
										$this->Idistribute_model->create_dist ( $update_dist );//更新分销信息
										$this->Idistribute_model->update_ext_grade ( $update_dist );//更新泛分销信息
...
// 											'orderid' => $orderid,
						...
						if ($order ['complete_balance_given'] == 2) {
							...
							if (! empty ( $balance_info ['give_amount'] )) {
								...
								$balance_reward = $this->Member_new_model->addBalance($inter_id,$order ['openid'],$order ['orderid'],$balance_info ['give_amount'],'离店返现');//余额退款处理
								$this->db->where ( array (
										'orderid' => $order ['orderid'],
										'inter_id' => $inter_id 
								) );
...
//        $this->db->insert('weixin_text',array('content'=>'order_weixin_refund+订单号：'.$order ['orderid'].'退款失败','edit_date'=>date('Y-m-d H:i:s')));
                        ...
                        if($order['paytype'] == 'unionpay' && $order['paid'] ==1 ){
                        	...
                        	if(isset($config_data) && $config_data['CHECK_UNIONPAY_REFUND']==1){
				...
				case 5 :
                    ...
                    if($vid==1){
						...
						//暂时只支持全额支付退还
						if($status==5){
							if($order['paytype'] == 'balance' && $order['balance_part'] == 0 && $order['paid'] == 1){
								$balance_cfg = $this->Hotel_config_model->get_hotel_config($order ['inter_id'], 'HOTEL', 0, array(
									'CANCEL_ORDER_BACK_BALANCE'
								));
								
								if(isset($balance_cfg['CANCEL_ORDER_BACK_BALANCE']) && $balance_cfg['CANCEL_ORDER_BACK_BALANCE'] == 1){
									if($order ['point_used'] == 1){
										$balance_back = $this->Member_model->refundBalance($inter_id, $order['orderid'], $order['price']);
										if($balance_back){
											$this->update_balance_refund($order['inter_id'], $order['orderid'], 1);
										}else{
											$this->update_balance_refund($order['inter_id'], $order['orderid'], 0);
										}
									}
								}
							}
				...
				case 'ss' : // 下单成功
					...
					foreach ( $order ['order_details'] as $od ) {
						...
						$I_params = array(
							...
							'order_time' => date('Y-m-d H:i:s',$order ['order_time']), //下单时间
...
// 					}
				...
				//暂时只支持全额支付退还
				if($status==5){
					if($main_order['paytype'] == 'balance' && $main_order['balance_part'] == 0 && $main_order['paid'] == 1){
						$balance_cfg = $this->Hotel_config_model->get_hotel_config($main_order ['inter_id'], 'HOTEL', 0, array(
							'CANCEL_ORDER_BACK_BALANCE'
						));
						
						if(isset($balance_cfg['CANCEL_ORDER_BACK_BALANCE']) && $balance_cfg['CANCEL_ORDER_BACK_BALANCE'] == 1){
							
							$real_allprice=$this_item['real_allprice'];
							$back_price=array_sum($real_allprice);
							if($main_order ['point_used'] == 1){
								$balance_back = $this->Member_model->refundBalance($inter_id, $main_order['orderid'], $back_price);
								if($balance_back){
									$this->update_balance_refund($main_order['inter_id'], $main_order['orderid'], 1);
								}else{
									$this->update_balance_refund($main_order['inter_id'], $main_order['orderid'], 0);
								}
							}
						}
					}
...
//         if (! empty ( $main_order ['coupon_des'] )) {//优惠券退回
//                $this->load->model ( 'hotel/Coupon_model' );
//                $order_wxcards = $this->Coupon_model->change_order_coupon ( $main_order ['orderid'], $main_order ['inter_id'], $main_order ['openid'], $main_order ['coupon_des'], 'back' ,1,$main_order);
//            }
	...
	public function get_order_item($inter_id, $orderid, $item_id = null, $detail = null) {
		...
		$this->db->from ( self::TAB_HOI . ' i' );
		if($detail == 'detail'){
			$this->db->select ( 'h.name hotelname,i.*,o.name,o.tel,o.order_time,o.paytype' );
			$this->db->join ( self::TAB_HO . ' o', 'o.inter_id=i.inter_id and o.orderid=i.orderid' );
			$this->db->join ( self::TAB_H . ' h', 'h.inter_id=i.inter_id and h.hotel_id=o.hotel_id' );
		}
		if (! empty ( $item_id )) {
	...
	function order_stock($inter_id, $hotel_id, $room_id, $condits) {
		...
		$this->db->where_in ( 'date', $condits ['day_range'] );
		$this->db->where_in ( 'price_code', array (
				'-1',
				$condits ['price_code'] 
		) );
		$price_codes = $this->db->get ( self::TAB_HRS )->result_array ();
		$date_num = array ();
	...
	function pay_return($orderid, $openid = '') {
		...
		$order = $this->db->get ( self::TAB_HO )->row_array ();
		if ($order) {
			if ($order ['paid'] == 1) {
				...
				if (! $payresult) {
					...
					if (! empty ( $order )) {
						...
						if (! empty ( $config_data ['PMS_AFT_SUBMIT'] ) && $config_data ['PMS_AFT_SUBMIT'] == 1) {
							...
							if ($result ['s'] == 0) {
								$this->handle_order ( $order ['inter_id'], $order ['orderid'], 10, $order ['openid'] ); // pms下单失败，退回
							}
						} else {
							...
							if ($status==1){
							    $this->handle_order ( $order ['inter_id'], $orderid, 1, $order ['openid'] );
							}else if ($status==0){
...
//				$this->Order_queues_model->cancel_queue($order ['inter_id'],$order ['hotel_id'],$order ['orderid']);
	...
	function get_lowest_price_date($inter_id, $condits) {
		...
		$valid_sql = 'select price_code from ' . $this->db->dbprefix ( self::TAB_HPS ) . " where inter_id = '$inter_id' and status=1 $h";
		...
		$sql = "select a.hotel_id,min(format(sprice/dnum,2)) lowest from 
				(SELECT sum(price) sprice,hotel_id,price_code,count(date) dnum ,room_id FROM `" . $this->db->dbprefix ( self::TAB_HRS ) . "` 
	...
	function get_lowest_price($inter_id, $condits) {
		...
		if($hotel && empty($condits['price_codes'])){//活动页面不读缓存
			//读取Redis数据
			$_hotels=[];//没有缓存数据的酒店ID
			foreach($hotel as $v){
				$_lowest=lowest_from_redis($inter_id,$v,$condits['member_level'],$condits['startdate'],$condits['enddate']);
				if($_lowest!==null){
					$data[$v]=number_format($_lowest,2,'.','');
				}else{
					$_hotels[]=$v;
		...
		if(!empty($condits['price_codes'])){
			$_hotels=$hotel;
		}
		//_hotels为NULL或_hotels是非空数组
		if($_hotels === null || (is_array($_hotels) && $_hotels) || $condits['price_codes']){
	...
	function get_lowest_code_price($inter_id,$hotel_ids=[],$price_codes=null) {
		$this->db->select ( 'min(s.price) m_price,s.hotel_id' );
		$this->db->from ( self::TAB_HPS . ' s' );
		$this->db->join ( self::TAB_HPI . ' i', 's.inter_id=i.inter_id and s.price_code=i.price_code' );
		$this->db->join ( self::TAB_HR . ' r', 'r.inter_id=i.inter_id and r.room_id=s.room_id and r.hotel_id=s.hotel_id' );
		$this->db->where ( array (
				's.inter_id' => $inter_id,
				's.price >' => 0,
				's.status' => 1,
		...
		if(!empty($price_codes)){
			$this->db->where_in( 's.price_code',$price_codes );
		}
		//只查询传入的酒店
		if($hotel_ids){
			$this->db->where_in('s.hotel_id',$hotel_ids);
		}
	...
	public function get_tmp_lowest_price($inter_id,$hotel_ids=[]) {
		$this->db->from('hotel_lowest_price')->where(array (
			                                             'inter_id' => $inter_id
		                                             ));
		//只查询传入的酒店
		if($hotel_ids){
		...
		$lowest=$this->db->get()->result_array();

		/*$lowest = $this->db->get_where ( 'hotel_lowest_price', array (
				'inter_id' => $inter_id 
		) )->result_array ();*/
		$tmp_mins = array ();
		foreach ( $lowest as $l ) {
	...
	function update_balance_refund($inter_id,$order_id,$status=1){
