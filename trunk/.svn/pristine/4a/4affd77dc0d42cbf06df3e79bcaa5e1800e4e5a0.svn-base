...
class Subaapi_webservice{
	 ...
	 */
	public function AddComment($OrderID,$CustomerID ,$Result,$Content,$Feels,$IpAddress){
...
//		LOG::pms_access_record($inter_id, $wait_time, $record_type, $url.'/'.$record_type, $send_content, $receive_content, $open_id);
		/* $now = time ();
		$s = doCurlGetRequest ( $url );
		$mirco_time = microtime ();
		$mirco_time = explode ( ' ', $mirco_time );
