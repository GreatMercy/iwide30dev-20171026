...
class Idistribution_model extends MY_Model {
	...
	public function get_best($params){
		...
		//有链接分销员id才执行
		if($params['link_saler'] || $params['own_saler'] || $params['link_f_saler'] || $params['own_f_saler'] ){
			//初始化查询条件
			$db_read->reset_query();
			$sqk_sql = $sql."AND beyond=3 ORDER BY weight desc LIMIT 0,1";//推荐归属
			$row3 =  $db_read->query ( $sqk_sql )->row_array ();
			$db_read->reset_query();
			$sqk_sql = $sql."AND beyond=4 ORDER BY weight desc LIMIT 0,1";//泛分销推荐归属
			$row4 =  $db_read->query ( $sqk_sql )->row_array ();

			if( $row3 && ($params['own_saler'] || ($params['link_saler'] && ( !$params['own_f_saler'] || ($params['own_f_saler'] && !$row4) ) ) ) ){

				$money = $this->get_count($row3['excitation_type'],$params['grade_amount'],$params['days'],$row3['excitation_value'],$row3['hotel_value'],$row3['group_value'],$row3['jfk_value']);
				if($money){
					$data = array (
						"inter_id" => $params['inter_id'],//公众号ID
						"hotel_id" => $params['hotel_id'],//下单酒店ID
						...
						"grade_amount" => $params['grade_amount'],//订单计算绩效部分的金额
						"status" => 4,//未核定－尚未离店
						"grade_amount_rate" => ($row3['excitation_type']==1)? bcdiv($row3['excitation_value'],100,4) : $row3['excitation_value'],//绩效值/比例（grade_total等于-1，此字段可以不传）
						"grade_rate_type" => ($row3['excitation_type']+1),//订房激励2：优惠后房价百分比3：每个订单激励固定金额4：每间/夜激励固定金额
						"remark" => "订房下单生成分销信息",//备注
						"product" => $params['product'],//产品名称
						"order_status" => $params['istatus'],//订单状态
						"fans_hotel" => $params['fans_hotel'],//粉丝所属酒店
						"hotel_rate" => ($row3['excitation_type']==1)? bcdiv($row3['hotel_value'],100,2) : $row3['hotel_value'],//酒店绩效规则值
						"group_rate" => ($row3['excitation_type']==1)? bcdiv($row3['group_value'],100,2) : $row3['group_value'],//集团绩效规则值
						"jfk_rate" => ($row3['excitation_type']==1)? bcdiv($row3['jfk_value'],100,2) : $row3['jfk_value'],//金房卡绩效规则值
						"hotel_grades" => $money['hotel'],//酒店绩效金额
						"group_grades" => $money['group'],//集团绩效金额
						"jfk_grades" => $money['jfk'],//金房卡绩效金额
					...
					//分销员下单绩效总归自己
					if($params['own_saler']>0){
						$data['saler'] = $params['own_saler'];
					}
					$this->Idistribute_model->create_dist ( $data );//创建数据
				}
			}elseif( $row4 && ($params['own_f_saler'] || ($params['link_f_saler'] && ( !$params['own_saler'] || ($params['own_saler'] && !$row3) ) ) ) ){
				$money = $this->get_count($row4['excitation_type'],$params['grade_amount'],$params['days'],$row4['excitation_value'],$row4['hotel_value'],$row4['group_value'],$row4['jfk_value']);
				if($money){
					$grade_rule = $this->enums('excitation_type')[$row4['excitation_type']] . ':';
					$grade_rule .=($row4['excitation_type']==1)? bcdiv($row4['excitation_value'],100,2).'%' : $row4['excitation_value'].'元';
					$data = array (
						"inter_id" => $params['inter_id'],//公众号ID
						"hotel_id" => $params['hotel_id'],//下单酒店ID
						"saler" => $params['link_f_saler'],//分销号，粉丝归属的分销号不用传，按次计算需要传
						"grade_openid" => $params['grade_openid'],//粉丝openid
						"grade_table" => "iwide_hotels_order",//订房的订单表
						"grade_id" => $params['grade_id'],//记录产生绩效的表的主键值
						"order_amount" => $params['order_amount'],//订单总金额（包含优惠折扣金额）
						...
						"grade_amount" => $params['grade_amount'],//订单计算绩效部分的金额
						"status" => 4,//未核定－尚未离店
						// "grade_amount_rate" => ($row4['excitation_type']==1)? bcdiv($row4['excitation_value'],100,2) : $row4['excitation_value'],//绩效值/比例（grade_total等于-1，此字段可以不传）
						// "grade_rate_type" => ($row4['excitation_type']+1),//订房激励2：优惠后房价百分比3：每个订单激励固定金额4：每间/夜激励固定金额
						"remark" => "订房下单生成分销信息",//备注
						'order_hotel'   => $params['hotel_id'],
						"order_status" => $params['istatus'],//订单状态
						'order_time'    => $params['order_time'],
						'product_count' => $params['days'],
						"product" => $params['product'],//产品名称
						'grade_rule'    => $grade_rule,
						'grade_typ'     => 2
					);
					//分销员下单绩效总归自己
					if($params['own_f_saler']>0){
						$data['saler'] = $params['own_f_saler'];
					}
					$this->Idistribute_model->create_ext_grade ( $data );//创建全民分销数据
				}
			}
		}
	...
	*/
	public function leave_recount($inter_id,$grade_id,$grade_amount,$days,$order_status,$olddays){
		...
		$old = $this->Idistribute_model->is_grades_exist($inter_id,$grade_id,"iwide_hotels_order",2,true);//查询下单时信息(泛分销)
		//如果有对应分销信息
		if($old){
			if($grade_amount != $old->grade_amount){//如果价格与下单时不同，才重新计算
				$excitation_type_arr = $this->enums('excitation_type');
				$grade_rule = explode(':',$old->grade_rule);
				foreach ($excitation_type_arr as $tk => $tv) {
					if($tv == $grade_rule[0]){
						$excitation_type = $tk;
					}
				}
				if(empty($excitation_type)){
					return false;
				}
				if($excitation_type == 1){//百分比
					$grade_amount_rate = str_replace('%','',$grade_rule[1]) * 100;
				}
				$money = $this->get_count($excitation_type,$grade_amount,$days,$grade_amount_rate,0,0,0);
				$data['grade_total'] = $money? $money['saler']:0;//绩效总金额
				$data['order_amount'] = $grade_amount;//订单总金额（包含优惠折扣金额）
				$data['grade_amount'] = $grade_amount;//订单计算绩效部分的金额
			}
			$data['grade_typ'] = 2;//泛分销员归属
			$this->Idistribute_model->update_ext_grade ( $data );//更新分销信息
		}
		$this->write_log($old,$data,'泛分销');
