...
<div class="martop pad3 webkitbox input_item bg_fff bd" style="-webkit-box-align:baseline; padding-right:0">
    ...
    <?php if(!empty($pay_ways)) foreach($pay_ways as $k=>$pw){?>
        <div pay_type='<?php echo $pw->pay_type;?>' class="pay_way <?php if($k==0){?>ischeck<?php }?>">
            <tt class="color_main"><em class="iconfont">&#x4f;</em></tt>
    ...
    <div class="relative" style="width:100%; height:100%;">
        <div class="scroll absolute bg_fff">
            ...
            <div class="list">
                ...
                <div class="item webkitbox bd" youhui><span>优惠劵</span><span class="color_main" id="list_total_coupon_price">-￥0</span></div>
                <div detail></div>
		...
		if($('.price_detail_pull').is(':hidden')){
			...
			for(var i=0;i<num;i++){
				...
				str += '<div class="item webkitbox"><span>'
				    + date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate()+'</span><span>'
					+ '<?php if(!empty($first_room['room_info']['imgs']['hotel_room_service']))foreach($first_room['room_info']['imgs']['hotel_room_service'] as $hs)echo $hs['info'].' ';?>'
				    + '</span><span>￥'+total_price+'X'+$('#roomnum').val()+'</span></div>';
			}
			$('[fangfei]').next('[detail]').html(str);
			$('#list_total_price').html('￥'+real_price.toFixed(2));
			//优惠券
			str='';
			...
			$('#list_total_coupon_price').html('-￥'+total_favour);
		...
		$.post('/index.php/hotel/hotel/return_usable_coupon?id=<?php echo $inter_id;?>',{
			...
			if(data.cards!=''){	
				...
				$.each(data.cards,function(i,n){
					...
					temp+='<p class="color_888">'+n.brand_name+'</tt>';
