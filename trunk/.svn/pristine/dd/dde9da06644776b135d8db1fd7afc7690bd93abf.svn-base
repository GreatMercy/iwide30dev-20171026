			...
			if(count( $room['TeamBuyRooms']) > 0){
				...
				if( count( $discount_price_array[0] ) > 0 ){
					...
					if( isset( $temp_state_info['least_num'] ) ){
						...
						//取团购的state_info
						$discount_state_array = $this->getDiscountPriceState($discount_price_array, $buy_num, $pre_pay, $closeRoom, $coupon_limit, $least_room_num,$temp_state_info['des'],$RoomGuaranty,$temp_state_info['extra_info']);
...
// 			$returnData[$room_id]['show_coupon'] = 1;
	...
	private function getPayLogByOrderNo($order_no){
		...
		$sql = "
				SELECT rtn_content
				FROM
					".$this->db->dbprefix ( self::TAB_PAY_LOG )."
				WHERE
	...
	public function getSaler($inter_id,$grade_openid){
		...
		$saler_data = $this->db->query ( $sql )->result_array ();
		...
		if($saler_data['saler']){
			...
			$saler_all_data = $this->db->query ( $sql )->result_array ();
	...
	public function addRegisterDistribute($inter_id,$openid,$member_add_id,$member_no){
		...
		if($have_num < 1){
			...
			$this->addDistributeForAddMember(0.2,$inter_id,$saler_data['saler'],$openid,$member_id
	...
	function check_order_canpay($order, $pms_set) {
		$web_order = $this->get_order_info ( $order, $pms_set);
		if(isset($web_order['OrderStatus']) && ($web_order['OrderStatus'] == 1 || $web_order['OrderStatus'] == 0)){//订单预定或确认
			return true;
		}else{
			return false;
		}
