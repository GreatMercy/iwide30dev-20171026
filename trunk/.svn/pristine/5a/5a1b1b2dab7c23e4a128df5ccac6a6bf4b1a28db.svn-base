...
//		$data ['startdate'] = date ( 'Y/m/d' );
	...
	function index() {
		    ...
		    if (!empty($r['state_info'])){
		        foreach ($r['state_info'] as $state_key=>$state){
		           $data ['rooms'][$room_key]['state_info'][$state_key]['des']=htmlblank_replace($state['des']);
                    ...
                    if(!empty($room_pay_way))$coupon_paytype = current($room_pay_way)->pay_type;else $coupon_paytype = 'daofu';
		...
		if($tc_id>0){
			...
			if(!empty($tc_row)){
				...
				foreach ($data ['rooms'] as $tc_k => $tc_v) {
					foreach ($tc_v['state_info'] as $tc_kk => $tc_vv) {
						if( !in_array($tc_vv['price_code'],$tc_price_codes) ){
							unset($data ['rooms'][$tc_k]['state_info'][$tc_kk]);
						}elseif(!isset($tmp_lowest_price)){
							$tmp_lowest_price = $tc_vv['avg_price'];
						}elseif($tmp_lowest_price > $tc_vv['avg_price'] ){
							$tmp_lowest_price = $tc_vv['avg_price'];
						}
					}
					if(empty($data ['rooms'][$tc_k]['state_info'])){
						unset($data ['rooms'][$tc_k]);
					}else{
						$data ['rooms'][$tc_k]['lowest'] = $tmp_lowest_price;
						unset($tmp_lowest_price);
					}
				}
			}
		}
	...
	function return_more_room() {
		...
		if($tc_id>0){
			...
			if(!empty($tc_row)){
				$tc_price_codes = json_decode($tc_row['price_codes'],TRUE);
				foreach ($rooms as $tc_k => $tc_v) {
					foreach ($tc_v['state_info'] as $tc_kk => $tc_vv) {
						if( !in_array($tc_vv['price_code'],$tc_price_codes) ){
							unset($rooms[$tc_k]['state_info'][$tc_kk]);
						}
					}
					if(empty($rooms[$tc_k]['state_info'])){
						unset($rooms[$tc_k]);
					}
				}
			}
...
//        $enddate = '2017/06/16';
//        $data ['price_codes'] = json_encode(array(11=>1));
//        $data ['price_type'] = json_encode(array('common'=>1));
//        $datas = json_encode(array(11=>1));
		...
		if (!empty($data ['first_state']['goods_info']['items'])){
		    if (!empty($data ['first_state']['is_packages'])&&empty($package_info)&&$data ['first_state']['goods_info']['sale_way']!=2){
    		    ...
    		    $package_check = $this->Goods_order_model->check_order_package($this->inter_id,$data ['first_state']['goods_info'],$package_info,array('startdate'=>$data ['startdate'],'enddate'=>$data ['enddate'],'roomnums'=>$order_data['roomnums']));
...
// 		) );
        ...
        $coupon_pay_ways = $data['pay_ways'];

        usort($coupon_pay_ways, function ($a, $b){
            return $b->sort != $a->sort?$b->sort - $a->sort:0;
        });
        ...
        $coupon_condition = array (
            ...
            'paytype'=>current($coupon_pay_ways)->pay_type
...
//                保存上传完之后的URL
	...
	public function room_list(){
        ...
        $this->display('hotel/bigger/room_list',$data);
	}
    // 套餐列表
	public function package_list(){
        $data['inter_id'] = $this->inter_id;

        $this->display('hotel/bigger/package_list',$data);
	}
	...
	public function submit_order(){
        ...
        $this->display('hotel/bigger/submit_order',$data);
	}
    // 我的订单
    public function my_order(){
        $data['inter_id'] = $this->inter_id;

        $this->display('hotel/bigger/my_order',$data);
    }
    ...
    public function order_details(){
        ...
        $this->display('hotel/bigger/order_detail',$data);
    }

    // 订单详情
    public function packages_use(){
        $data['inter_id'] = $this->inter_id;
        $this->display('hotel/bigger/packages_use',$data);
    }
    // 房型+套餐详情
    public function package_details(){
        $data['inter_id'] = $this->inter_id;       
        $this->display('hotel/bigger/package_details',$data);
    }
	
    // 选择套餐
    public function package_select(){
        $data['inter_id'] = $this->inter_id;       
        $this->display('hotel/bigger/package_select',$data);
    }

    //城市列表
    public function city_list(){
        $data['inter_id'] = $this->inter_id;       
        $this->display('hotel/bigger/city_list',$data);
    }

    // 评价页面
    public function comment_list(){
        $data['inter_id'] = $this->inter_id;       
        $this->display('hotel/bigger/hotel_reviews',$data);
    }

    // 发表评价
    public function comment(){
        $data['inter_id'] = $this->inter_id;       
        $this->display('hotel/bigger/to_comment',$data);
    }

    // 酒店介绍
    public function hotel_details(){
        $data['inter_id'] = $this->inter_id;       
        $this->display('hotel/bigger/hotel_details',$data);
    }
    ...
    public function map(){
        $data['inter_id'] = $this->inter_id;       
        $this->display('hotel/bigger/map',$data);
    }
