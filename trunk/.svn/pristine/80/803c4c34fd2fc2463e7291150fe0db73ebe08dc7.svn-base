...
// 							$si ['least_num'] = $web_state ['least_num'];
	...
	function order_to_web($inter_id, $orderid, $paras = array(), $pms_set = array()) {
		...
		if (! empty ( $order )) {
			...
			$zzmsg = isset ( $zzmsgarr [1] ) ? $zzmsgarr [1] [0] : '';
			$zzcode = isset ( $zzcodearr [1] ) ? $zzcodearr [1] [0] : '';
			$zzoid = isset ( $zzoidarr [1] ) ? $zzoidarr [1] [0] : '';
			
			// ////////////////////////////////////////////////////////向API服务器提交信息成功后往下继续进行
			
			// 提交结束
			if ($zzcode == 0) { // 提交成功，把订单数据保存到本地
				$web_orderid = $zzoid;
				$this->db->where ( array (
				...
				if ($order ['status'] != 9) {
					...
					if (! empty ( $config_data ['PMS_BANCLANCE_REDUCE_WAY'] ) && $config_data ['PMS_BANCLANCE_REDUCE_WAY'] == 'after') {
						$this->load->model ( 'hotel/Member_model' );
						if ($this->Member_model->reduce_balance ( $inter_id, $order ['openid'], $order ['price'], $order ['orderid'], '订房订单余额支付',array(),$order)) {
