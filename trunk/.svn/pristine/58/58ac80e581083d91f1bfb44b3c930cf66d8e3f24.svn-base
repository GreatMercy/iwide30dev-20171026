...
class Advert_model extends CI_Model {
	...
	public function get_hotel_ads($inter_id, $hotel_id = 0, $area_type, $status = null, $ad_status = null) {
		...
		$this->db->where_in ( 'hotel_id', $in );
		is_null ( $status ) || $this->db->where ( 'status', $status );
		$result = $this->db->get ( self::TAB_AD_AREAS )->result_array ();
		$data = array ();
		if (! empty ( $result )) {
	...
	public function get_ad_by_ids($inter_id, $hotel_id = 0, $ids = null, $ad_status = null ,$type='') {
		...
		is_null ( $ad_status ) ? $this->db->where_in ( 'status', array (
				1,
				2 
		...
		if($type=='index_middle'){
			$this->db->like('ad_img', '&#', 'after');
		}
		if($type=='index_foot' || $type=='search_foot'){
			$this->db->like('ad_img', 'http', 'after');
		}
		return $this->db->get ( self::TAB_ADS )->result_array ();
	}
	function get_ad_area_list($inter_id, $module, $hotel_id = 0, $code = '') {
		$area_type = $this->get_module_type_name ( $module );
		$area_sql = 'select * from ' . $this->db->dbprefix ( self::TAB_AD_AREAS ) . " where inter_id='$inter_id' and hotel_id=$hotel_id";
		$type_sql = 'select * from ' . $this->db->dbprefix ( self::TAB_ENUM ) . " where type='$area_type' and status=1";
		if($hotel_id>0){
	...
	public function get_ad_area($inter_id, $hotel_id, $area_type) {
		...
		return $this->db->get ( self::TAB_AD_AREAS )->row_array ();
	}
