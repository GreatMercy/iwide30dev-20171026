...
// 					if($this->_db ( 'iwide_rw' )->insert ( 'openid_inter_id_rel', $params ) ){
	...
	public function update_rel_info($params = array()) {
		...
		if (isset ( $params ['minter_id'] ))
			$where_params ['minter_id'] = $params ['minter_id'];
		$this->_db ( 'iwide_r1' )->where ( $where_params );
		$fields = $this->_db ( 'iwide_r1' )->list_fields ( 'openid_inter_id_rel' );
