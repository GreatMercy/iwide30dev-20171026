        ...
        <div class="form_item bd_bottom padding_18" v-for="(value,key) in configList"  v-if="value.show === 1" :data="value.show">
          <div class="flex">
            ...
            <div v-if="key === 'sex'" class="flex_1">
              <select class="font_15" :value="value.value" :name="key" :placeholder="value.note" :disabled="view">
            ...
            <div v-else class="flex_1">
              <input class="font_14"  :disabled="view" @keyup="setRemove($event)" :value="value.value" :type="value.type" :name="key" :placeholder="value.note">
            </div>
          </div>
        </div>
        ...
        if (this.configList[item]['show'] === 1) {
          switch (item) {
            case 'name':
              ...
              break
            case 'phone':
              this.configList[item].value = this.centerInfo.telephone
              break
            case 'email':
            ...
            case 'sex':
              ...
              break
            case 'birthday':
              this.configList[item].value = formatDate(this.centerInfo.birth)
              break
            case 'idno':
