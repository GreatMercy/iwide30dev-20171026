...
<!--            <span>可使用积分10000 抵¥100</span>-->
            <span id="userReduceTips">可使用<?php if( $this->inter_id == 'a472731996' ) echo '雅币'; else echo '储值';  ?></span>
            <span id="ra">
			...
			if(data.status == <?php echo Soma_base::STATUS_TRUE;?>){
				...
				if(discountData.asset.status == <?php echo Soma_base::STATUS_TRUE;?>){
					...
					if(asset.rule_type == <?php echo $salesRuleModel::RULE_TYPE_POINT;?>){
						...
						$('#userReduceTips').html('可使用<?php if( $this->inter_id == "a472731996" ) echo "雅币"; else echo "储值";  ?>¥'+ asset.quote );
						$('#passwordText').show();
					}
...
function showCalcTotalHtml(price){
	...
	$('#userReduceTips').html( '可使用<?php if( $this->inter_id == "a472731996" ) echo "雅币"; else echo "储值";  ?>¥'+total.toFixed(2) );
