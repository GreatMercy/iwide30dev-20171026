...
<a href="<?php echo $fa['ad_link'];?>" class="pad3 bg_fff bd h24 martop color_link" style="display:block"><em class="iconfont"><?php if(!strstr($fa['ad_img'],"http")) echo $fa['ad_img'];?></em><?php echo $fa['ad_title'];?></a>
<?php }}?>
...
<div class="box_room_list">
	...
	<?php if($cur_label_count>0){if($label_output_count==0){?>
		<div class="webkitbox justify bd_bottom pad3" onClick="item_silde_down(this,'room_type_label_<?php echo $r['room_info']['type_label']['id']?>')">
    		...
    		<em class="iconfont color_main rotate torotate">3</em>
    	</div>
		...
		<div class="item" onClick="silde_down(this)">
	        ...
	        <div class="item_foot"  <?php if(!$cur_label_count&&$i>0){?>style="display:none;"<?php }?>>
	        	...
	        	<?php foreach($r['show_info'] as $rsi) {?>
	               ...
	               <?php  }?>
	            <div class="bd_bottom list_style_2" style="background:none">
	            <?php if(!empty($r['state_info'])){ foreach($r['state_info'] as $si){?>
	            	<div class="webkitbox justify pad3">
	                	<div style="max-width:50%">
							<p>
                                <?php echo $si['price_name'];?> <span class="btn_void xs" style="font-size:8px; color:#7f9fe9; vertical-align:top">社群客</span>
                                <?php if(isset($si['useable_coupon_favour']) && !empty($si['useable_coupon_favour'])){ ?>
                                    <span class="btn_void xs" style="font-size:8px; color:#7f9fe9; vertical-align:top">
                                <?php echo '可用'.$si['useable_coupon_favour'].'元券'; }
                                  if($si['wxpay_favour_sign']==1){?>
                                    <span class="btn_void xs" style="font-size:8px; color:#7f9fe9; vertical-align:top">
                                    <?php echo '立减'.$si['bookpolicy_condition']['wxpay_favour'].'元';?>
                                </span>
                                <?php    }elseif (!empty($si['price_tags'])){foreach ($si['price_tags'] as $tag){?>
                                    <span class="btn_void xs" style="font-size:8px; color:#7f9fe9; vertical-align:top">
                                    <?php echo $tag;?>
                                </span>
                                <?php }}?>
                            </p>
	                        <div class="h24 color_main">无早</div>
	                    	<div class="h24 color_999"><?php echo $si['des'];?></div>
	                    </div>
	                    <div class="webkitright">
	                    	<?php if($si['book_status']=='available'&&$si['avg_price']>=0) {?>
       ...
       <div class="center h20" style="padding:30px;">
	   		<div>没有搜索到相关结果，换个日期试试</div>
            <div>或拨打客服电话</div>
            <a class="color_link" href="tel:4008289966">400-828-9966</a>
	   </div>
       <?php }?>
	</div><!--room_list --->
</div><!--box_room_list --->
...
function initHtml(data,bol){
	...
	var temp='';
		...
		$.each(data.rooms,function(i,n){
			...
			if(cur_label_count>0){
				if(label_output_count==0){
					...
					temp+=' <div class="h26 border_left_2">'+n.room_info.type_label.label_name+'</div><em class="iconfont color_main rotate torotate">3</em>';
					temp+='</div>';
			...
			if(n.state_info!=''){
				...
				if(n.lowest>=0)temp+='<tt class="h20">¥</tt><tt class="h36">'+n.lowest+'</tt><tt class="h20 color_999">起</tt>';
			...
			<?php }?>
			temp+='<div class="bd_bottom list_style_2" style="background:none">';
			if(n.state_info!=''){
				$.each(n.state_info,function(sk,sn){
		...
		if(temp==''||data.errmsg){
			temp='<div class="center h20" style="padding:30px;"><div>没有搜索到相关结果，换个日期试试<div>或拨打客服电话</div><a class="color_link" href="tel:4008289966">400-828-9966</a></div>'
		}
		$('#room_list').html(temp);
		removeload();
}
