            	...
            	<div class="webkitbox justify pad3">
                    ...
                    <?php if (!empty($si['total_point'])){?>
						...
						<?php } else{?>
								...
								<?php }else{?>
		                        	<a href="<?php echo site_url('membervip/login').'?id='.$inter_id.'&redir='.urlencode($_SERVER ['HTTP_HOST'] . $_SERVER ['REQUEST_URI']);?>"><span style="float:right" now class="bg_minor ">登录</span></a>
								<?php }?>
							<?php }?>                    
                        <?php } else{?>
								...
								if(sn.book_status=='available'){
									if(bonus>=sn.total_point){
										...
										if(is_login){
											...
											temp+='<a href="<?php echo site_url('membervip/login').'?id='.$inter_id;?>"><span style="float:right" now class="bg_minor">登录</span></a>';
									}
								}
								else
