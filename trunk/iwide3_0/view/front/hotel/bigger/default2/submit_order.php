...
<div id="multi_inners" <?php if(empty($customer_condition['show_first'])){?>style='display:none'<?php }?>>
    ...
    <?php if (!empty($customer_condition['child']['num'])){for ($i=1;$i<=$customer_condition['child']['num'];$i++){ ?>
        ...
        <div class="input_item relative">
       		<span>出生日期</span>
            <div>
            	<input type="date" key='extra_formdata' min_year='<?php echo date('Y')-10;?>' max_year='<?php echo date('Y')-3;?>' key_type='multi_inner' must='1' name='multi_inners_child_birthday[]' 
            	class="person multi_inners_child_birthday" placeholder="儿童生日" tips="儿童年龄为4~11岁，请重新选择生日" value="" style="position:absolute; width:100%; height:100%; opacity:0; left:0;right:0" onChange="$(this).siblings().val($(this).val());"/>
                <input class="tmp_birthday" placeholder="儿童生日" title="此input只显示内容，不做数据传输"/>
            </div>
        </div>
        <?php }?>
    <?php }}?>
      	...
      	<?php if (!empty($addit_service['transport']['pickup'])){?>
             ...
             <div name='addit_service_pickup' field='addit_service_pickup_train' style='display: none' class="input_item h22 arrow relative">
                ...
                <div style="-webkit-box-flex:1">
                	<input key='extra_formdata' key_type='addit_service' name='addit_service_pickup_traintime' placeholder="到达时间" type="datetime-local" style="position:absolute; width:100%; height:100%; opacity:0; left:0;right:0" onChange="$(this).siblings().val($(this).val());"/>
                    <input key='extra_formdata' key_type='addit_service' name='addit_service_pickup_traintime' placeholder="到达时间" title="此input只显示内容，不做数据传输"/></div>
             </div>
             <?php }?>
             ...
             <div name='addit_service_pickup' field='addit_service_pickup_airport' style='display: none' class="input_item h22 arrow relative">
             	<span>到达时间</span>
                <div style="-webkit-box-flex:1">
                	<input key='extra_formdata' key_type='addit_service' name='addit_service_pickup_plaintime' placeholder="到达时间" type="datetime-local" style="position:absolute; width:100%; height:100%; opacity:0; left:0;right:0" onChange="$(this).siblings().val($(this).val());"/>
                    <input key='extra_formdata' key_type='addit_service' name='addit_service_pickup_plaintime' placeholder="到达时间" title="此input只显示内容，不做数据传输"/></div>
    		</div>
    		<?php }?>
         <?php }?>
         <?php if (!empty($addit_service['transport']['takeoff'])){?>
             ...
             <?php if(!empty($addit_service['transport']['takeoff']['ways']['train'])){?>
                 ...
                 <div name='addit_service_takeoff' field='addit_service_takeoff_train' style='display: none' class="input_item h22 arrow relative show2_1">
                 	<span>到达时间</span>
                    <div style="-webkit-box-flex:1">
                    	<input key='extra_formdata' key_type='addit_service' name='addit_service_takeoff_traintime' placeholder="到达时间" type="datetime-local" style="position:absolute; width:100%; height:100%; opacity:0; left:0;right:0" onChange="$(this).siblings().val($(this).val());"/>
                        <input key='extra_formdata' key_type='addit_service' name='addit_service_takeoff_traintime' placeholder="到达时间" title="此input只显示内容，不做数据传输"/></div>
                 </div>
             <?php }?>
             <?php if(!empty($addit_service['transport']['takeoff']['ways']['airport'])){?>
                 ...
                 <div name='addit_service_takeoff' field='addit_service_takeoff_airport' style='display: none' class="input_item h22 arrow relative show2_2">
                 	<span>到达时间</span>
                    <div style="-webkit-box-flex:1">
                    	<input key='extra_formdata' key_type='addit_service' name='addit_service_takeoff_plaintime'  placeholder="到达时间" type="datetime-local" style="position:absolute; width:100%; height:100%; opacity:0; left:0;right:0" onChange="$(this).siblings().val($(this).val());"/>
                        <input key='extra_formdata' key_type='addit_service' name='addit_service_takeoff_plaintime' placeholder="到达时间" title="此input只显示内容，不做数据传输"/></div>
                 </div>
             <?php }?>
         <?php }?>
     <?php }?>
			...
			if(data.s==1&&data.consum_rate>0){
				if(part_bonus_set!=data.part_set){
                    ...
                    if(bonus_view ==1){
                        ...
                        $("#bonus_tips").html("可用"+point_max_use+point_name+"抵用"+ex_money+"元" + '<input type="hidden" id="bonus" name="bonus" value=""/>');
	...
	$('#roomnum').on('change',function(){
		...
		if(current_count>new_count){
			for(var i=0;i<(current_count-new_count);i++){
				$('#multi_inners > .multi_inner').last().remove();
			}
