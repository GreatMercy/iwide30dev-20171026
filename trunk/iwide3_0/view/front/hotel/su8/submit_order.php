...
<div class="ui_pull tips_box" id="over_tips" style="display:none">
	<div class="box">
        ...
        <div class="h4">由于您的到店时间较晚或预订房间较多，请您通过官网进行担保预订</div>
        <div class="btn_list">
...
////////////////////////初始化结束////////////////////////////
	...
	function Balance_pay(){ //余额+微信支付;
		if(rule.Balance>0 && rule.Balance<real_price){
			var _p = '余额支付('+rule.Balance+'元) + 微信支付('+(real_price-rule.Balance-total_favour).toFixed(2)+'元)';
			$('span','.pay_way[pay_type="balance"]').html(_p);
		}else{
			$('span','.pay_way[pay_type="balance"]').html('余额支付('+rule.Balance+'元)');
		}
	}
...
/* 原来的优惠券使用规则*/
                    ...
                    $.each(data.cards,function(i,n){
                        ...
                        temp+=' code='+n.code+' amount="'+n.reduce_cost+'" max_use_num="'+n.hotel_max_use_num+'" card_type="'+n.ci_id+'" use_num_type="'+n.hotel_use_num_type+'"><div class="checkbox"><em></em></div><div class="ui_vote"><p class="bordertop_img"></p><div class="vote_con">';
