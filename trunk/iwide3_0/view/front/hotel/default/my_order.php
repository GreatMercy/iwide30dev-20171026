...
<?php if(!empty($orders)){foreach($orders as $o){?>
	<div class="item" >
        ...
        <div class="normal foot">
        	<?php if($o['status']==3){ ?><a href="to_comment?id=<?php echo $inter_id;?>&oid=<?php echo $o['id'];?>" style="float:left" class="ui_color_gray">评价</a><?php }?>
        	<a href="orderdetail?id=<?php echo $inter_id;?>&oid=<?php echo $o['id'];?>" class=" ui_color_gray">订单详情</a>
        </div>
    </div>
    <?php }}else{?>
    <?php }?>
