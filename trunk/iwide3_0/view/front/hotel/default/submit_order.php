			...
			if (coupon_amount>0){
				$('#votelist .ischeck').each(function() {
					str += '<div class="item"><span>'
						...
						+ $('[name]',this).attr('name')+ '</span><span>'
						+$('[rebate]',this).text()+'</span></div>';
				});
			}else{
				str = '<div class="item">无</div>';
			}
			...
			if(data.cards!=''){	
				if(data.vid==undefined||data.vid==0){
					...
					$.each(data.cards,function(i,n){
						...
						temp+='<p rebate class="ui_price ui_color">'+n.reduce_cost+'元</p>';
						temp+='<p title><b>'+n.title+'</b></p>';
						temp+='<p class="ui_gray" name="'+n.brand_name+'">'+n.brand_name;
