...
<div class="webkitbox justify bg_fff">
	<div style="font-size:0"><img class="logo" src="<?php echo $config['img']?>"></div><!-- 酒店logo位-->
    <div class="pad3">
    	<!--
...
<input type="hidden" id="link" name="link" value="<?php echo site_url('hotel/check/near_hotel');?>" />
<div class="list_style index_list bd_bottom">
	<div class="webkitbox">
        ...
        <a href="<?php echo site_url('hotel/check/nearby')?>" class="center">
...
<div class="pad3 center ">
	<a href="http://whiteswan.iwide.cn/index.php/hotel/hotel/index?id=a451037398&h=1031" class="isable submitbtn bg_main" style="display:inline-block">白天鹅宾馆 快速预订通道</a>
</div>
<?php endif;?>
...
<!--  三个广告位 -->
<?php if(isset($config['open']) && $config['open'] == 2 && isset($config['menu']) && !empty($config['menu'])){?>
<style>
...
<div class="webkitbox others center">
	<?php foreach($config['menu'] as $k=>$v){?>
		<div class="<?php echo $v['code'];?>" <?php if($v['code'] == 'athour') echo 'href="'.site_url('hotel/hotel/search').'?id='.$inter_id.'&type=athour'.'"';?> <?php if($v['code'] == 'order') echo 'href="'.site_url('hotel/hotel/myorder').'"';?> <?php if($v['code'] == 'ticket') echo 'href="'.site_url('hotel/hotel/search').'?id='.$inter_id.'&type=ticket'.'"';?>>
	        ...
	        <div class="txtclip"><?php echo $v['desc'];?></div>
	        <div class="txtclip"><?php echo $select[$v['code']];?></div>
	    </div>
	<?php }?>
</div>
