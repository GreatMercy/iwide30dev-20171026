            ...
            <?php if(!empty($r['state_info'])){ foreach($r['state_info'] as $si){?>
            	<div class="webkitbox justify pad3">
                    ...
                    <?php if ($si['price_code']!=9){?>
                        ...
                        <?php } else{?>
                        <span style="float:right" now class="bg_999">满</span>
                        <?php }?>
                    <?php }?>
					...
					$.each(n.state_info,function(sk,sn){
						...
						if(sn.price_code!=9){
							if(sn.book_status=='available'){
								...
								temp+='<span style="float:right" now class="bg_999">满</span>';
						}
						temp+='<div class="color_minor">';
						if(sn.price_code==9){
							temp+='<del>¥'+parseFloat(sn.allprice)+'</del>';
						}else{
							temp+='<p>¥'+parseFloat(sn.allprice)+'</p><p class="h18">首晚价</p>';
						}
						temp+='</div></div></div>';
					});
				}
