...
<header style="padding-top:37px">
	<div class="headfixed webkitbox center pad3 bg_fff bd_bottom">
    	<a href="<?php echo site_url('hotel/hotel/myorder').'?id='.$inter_id.'&type='.$type.'&wfk=1'?>" class="<?php if($wfk==1){?>color_main<?php }?>">待付款订单</a>
    	<a href="<?php echo site_url('hotel/hotel/myorder').'?id='.$inter_id.'&type='.$type?>" class="bd_left <?php if($wfk===null){?>color_main<?php }?>">所有订单</a>
    </div>
</header>
<script>
...
<div class="martop">
    ...
    <a href="orderdetail?id=<?php echo $inter_id;?>&oid=<?php echo $o['id'];?>&type=<?php echo $o['price_type']?>" class="webkitbox h24 pad3">
    	<div class="img">
	...
	<?php if($o['status']==1 || $o['status']==2 || $o['status']==4 || $o['status']==5 || $o['status']==11){ ?>
        <a href="<?php echo site_url('hotel/hotel/index'); ?>?id=<?php echo $inter_id?>&h=<?php echo $o['hotel_id']?>&type=<?php echo $o['price_type']?>" class="pad10 bd_left color_888">再来一单</a>
    <?php }?>
    </div>
</div>
<?php }}?>
