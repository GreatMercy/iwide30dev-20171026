...
function rule_type_change_triger(){
	...
	if( rtv == "<?php echo $model::RULE_TYPE_REDUCE;?>" ){
		...
		var title= b_typ+ '_满 '+ $('#el_lease_cost').val()+ ' 立减 '+ $('#el_reduce_cost').val()+ '元现金';
		
	} else if( rtv == "<?php echo $model::RULE_TYPE_DISCOUNT;?>" ){
		$('#el_over_limit').parents('.form-group').show();
		$('#el_lease_cost').parents('.form-group').show();
		$('#el_reduce_cost').parents('.form-group').show();
		var title= b_typ+ '_满 '+ $('#el_lease_cost').val()+ ' 打 '+ $('#el_reduce_cost').val()+ '折';
		...
		var title= b_typ+ '_<?php $inter_id= $this->session->get_admin_inter_id(); if( $inter_id == "a472731996" ) echo "雅币"; else echo "储值";  ?>规则_ '+ ltv_l + '_'+ $('#el_limit_percent').val()+ '%';
