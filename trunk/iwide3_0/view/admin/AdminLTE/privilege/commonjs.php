...
var arrear_option = {
	...
	left :'好的<img src="http://file.cdn.iwide.cn/public/uploads/201612/qf281144376610.png">',   // 左按钮文字 默认为'取消'
	right:'-1'  // 右按钮文字 默认为'确定'
}
...
function notice(new_tips,tips){
	...
	if(new_tips>0){
		...
		$('<audio id="top_alter_order_ling"><source src="<?php echo base_url( FD_PUBLIC. '/'. $tpl. '/media/notify.ogg'); ?>" type= type="audio/ogg" ><source src="<?php echo base_url( FD_PUBLIC. '/'. $tpl. '/media/notify.mp3'); ?>" type="audio/mpeg"><source src="<?php echo base_url( FD_PUBLIC. '/'. $tpl. '/media/notify.wav'); ?>" type="audio/wav"></audio>').appendTo('body');
...
function center_notice(new_tips,tips){
	$('#notify_center_order').html('');
	if(new_tips>0){
		if(tips['room']!=0){
			var order_html = '';
			for (var i = 0; i < tips['room']; i++) {
				order_html += '<a href="javascript:to_check(1);">[待确认]您有一条新的订房订单需要确认哦！</a>';
			}
			$('#notify_center_order').html(order_html);
		}
		//$('#top_alter_order').attr('title', '您有新的订单！').attr('data-placement','left').tooltip('show');
		$('<audio id="top_alter_order_ling"><source src="<?php echo base_url( FD_PUBLIC. '/'. $tpl. '/media/notify.ogg'); ?>" type= type="audio/ogg" ><source src="<?php echo base_url( FD_PUBLIC. '/'. $tpl. '/media/notify.mp3'); ?>" type="audio/mpeg"><source src="<?php echo base_url( FD_PUBLIC. '/'. $tpl. '/media/notify.wav'); ?>" type="audio/wav"></audio>').appendTo('body');
		$('#top_alter_order_ling')[0].play();
	}
	$('#notify_center_num').html('消息中心（'+tips['room']+'未读）<i class="iconfont f_r close_btn" style="font-size:24px;">&#xe635;</i>');
	$('#notify_center_order_num').html(tips['room']);
	$('.news_radius').html(tips['room']);
	$('#top_alter_order> ul li.footer ').hide();	
	$('.close_btn').click(function(){
	    $('.j_toshow').animate({"right":"-330px"},300);
	});
}
...
function diylayer(option,handle,opt_delay_time,t_type,t_index){
	...
	var html='<style>.diylayer{position:fixed;z-index:999999;width:100%;height:100%;min-height:'+_h+'px;background:rgba(0,0,0,0.5);top:0;left:0;text-align:center}'
			+'.alertbox{margin:'+(_h-150)/2+'px auto;width:300px;height:150px;padding:20px;background:rgba(255,255,255,0.9);border-radius:7px;overflow:hidden}'
 ...
 $('.news').on('click',function(){
      $('.j_toshow').animate({"right":"0px"},300);
  });
