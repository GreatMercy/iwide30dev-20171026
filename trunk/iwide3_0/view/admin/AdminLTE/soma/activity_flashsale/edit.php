...
<link rel="stylesheet" href="<?php echo base_url(FD_PUBLIC) ?>/soma/flashsale/css/jedate.css">
<link rel="stylesheet" href="<?php echo base_url(FD_PUBLIC) ?>/soma/flashsale/css/style.css">
<!--<script src="--><?php //echo base_url(FD_PUBLIC) ?><!--/soma/killsec/js/jQuery-2.1.4.min.js"></script>-->
<script src="<?php echo base_url(FD_PUBLIC) ?>/soma/flashsale/js/jquery.jedate.min.js"></script>

<script src="<?php echo base_url(FD_PUBLIC) ?>/soma/flashsale/js/moment.min.js"></script>
<script src="<?php echo base_url(FD_PUBLIC) ?>/soma/flashsale/js/edit.js"></script>
...
<div class="main-title">
    <h1 class="f28"><?php echo ( $this->input->get('ids') ) ? '编辑': '新增'; ?>限时购</h1>
    <div class="line"></div>
</div>
     ...
     <div class="fl part">
        ...
        <div class="fl item f24 clearfix">
            <div class="radio-wrap">
                <input type="radio" value="1" name="activity_name" href="<?php echo Soma_const_url::inst()->get_url("*/activity_killsec/add"); ?>" class="homebuying-activity_name <?php echo ( $this->input->get('ids') ) ? 'is--disabled': ''; ?>">
                <span>限流模式</span>
            </div>

            <div class="radio-wrap">
                <input type="radio" value="2" name="activity_name" checked href="<?php echo Soma_const_url::inst()->get_url("*/*/add"); ?>" class="homebuying-activity_name <?php echo ( $this->input->get('ids') ) ? 'is--disabled': ''; ?>">
                <span>不限制流模式</span>
            </div>
        </div>
    </div>
...
<li class="clearfix" id="goodsInfo" style="display:block;">
    <div class="fl clearfix">
        <div class="fl sub-title f24"></div>
            <table class="goods-table-list" id ="goodsTableList">
                
            </table>
    </div>
</li>

<li class="clearfix" id="storeTypeTips">
    <div class="fl part">
        <div class="fl sub-title f24"><span class="required">*</span>限购方式</div>
        <div class="fl item f24 clearfix">
            <div class="radio-wrap">
                <input type="radio" value="1" name="is_astrict_buy" checked>
                <span>不限购</span>
            </div>
            <div class="radio-wrap">
                <input type="radio" value="2" name="is_astrict_buy">
                <span class="fl item">
                    <select name="" id="homebuyingType" class="choice-homebuying-model">
                        <option value="1">每个商品限购</option><option value="2">每个规格限购</option>
                    </select>
                </span>
                <div class="fl f24 limtNumber">
                    <input type="text" class="input f24 w200"  value=" <?php if(( isset($killSecInfo) && !empty($killSecInfo))) { echo ( ( strtotime($killSecInfo['end_time']) - strtotime($killSecInfo['killsec_time']) )/ 3600);}?>"  id="LimitShare">
                </div>
                <div class="fl f24" id="unit">份</div>
            </div>
        </div>
    </div>
</li>

<li id="storeTips" class="clearfix">
    <div class="fl sub-title"></div>
    <div class="tips f24 fl">注：选择不限购，则用户在有抢购库存的情况下，可以无限下单</div>
</li>
