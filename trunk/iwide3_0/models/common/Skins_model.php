...
class Skins_model extends CI_Model {
	...
	function get_skin_set($inter_id, $module, $hotel_id = 0) {
		$sql = 'select * from ' . $this->db->dbprefix ( self::TAB_SKIN_SET ) . '  
	...
	function get_module_skin($module, $skin_name) {
		return $this->db->get_where ( self::TAB_SKINS, array (
				'module' => $module,
				'skin_name' => $skin_name 
		) )->row_array ();
	}
	function get_disp_set($inter_id, $paras) {
		...
		$sql = "SELECT s.skin_name,s.overall_style,s.id skin_set_id,a.* from " . $this->db->dbprefix ( 'view_skin_set' ) . " s
