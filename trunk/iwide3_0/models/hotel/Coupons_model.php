			...
			if (!empty($rules[$c['card_id']])){
				foreach ($rules[$c['card_id']] as $r){
					...
					if (!empty($r['extra_rule'])){
						...
						//@Editor lGh 新加天数与数量限制 2016-8-26 20:28:08
						if (!empty($r['extra_rule']['max_room'])){
							$effects['nums_counts']=1;
							if($params['rooms']>$r['extra_rule']['max_room']){
								$checked[$c['card_id']]=0;
								break;
							}
						}
						if (!empty($r['extra_rule']['max_day'])){
							if($countday>$r['extra_rule']['max_day']){
								$checked[$c['card_id']]=0;
								break;
							}
						}
