...
class Coupons_rules_model extends MY_Model {
    ...
    function give_rules_list($inter_id){    //所有优惠券发放规则列表
        ...
        $res=$this->db->query("SELECT * FROM `iwide_hotel_coupon_grules` where inter_id='{$inter_id}'")->result();
...
//        return $db->update ( 'iwide_hotel_coupon_grules', $updata );
    ...
    function getAllCouponRules($inter_id,$status='1'){
        ...
        $res=$this->db->query("SELECT * FROM `iwide_hotel_coupon_grules` where inter_id='{$inter_id}' AND status={$status}")->result();
    ...
    function getSumCoupons($rule_id,$openid,$inter_id){
        ...
        $res=$this->db->query("SELECT `amount` FROM `iwide_hotel_coupon_count` where inter_id='{$inter_id}' AND openid='{$openid}' AND give_rule_id={$rule_id}")->row();
    ...
    public function getTotalOrders($inter_id,$openid){
        ...
        $res=$this->db->query("SELECT COUNT(id) as total FROM `iwide_hotel_orders` where inter_id='{$inter_id}' AND openid='{$openid}' AND status=3")->row();
...
//        var_dump($activeRules);exit;
    ...
    function add_user_coupon($inter_id, $order_id, $coupon,$add_type = 'create', $give_condition,$rule_id,$title,$openid,$give_num) {
        switch ($add_type) {
            case 'create' :
                $status = 4;
                break;
        ...
        foreach($give_info->$give_condition as $key=>$arr){
            ...
            foreach($coupon_ids as $coupon_id){
                for($x=1;$x<=$arr->card_nums;$x++){
                    ...
                    (isset($getNewCoupon['data']) && !empty($getNewCoupon['data']))?$status = 5:$status=6;
                    $res=$this->updateCouponStatus($order['inter_id'],$order['orderid'],$coupon_id,$key,$give_condition,$status);
                }
            }
            if($res)$update_coupon=$this->updateCouponCount($key,$order['openid'],$order['inter_id']);
        }
    ...
    function  getRuleOperateLog($inter_id,$ident){
        ...
        $res=$this->db->query("SELECT `log_id`,`record_time`,`admin`,`log_type`,`ip` FROM `iwide_hotel_admin_log` WHERE `inter_id`='{$inter_id}' AND `ident` = '{$ident}' ORDER BY record_time DESC")->result();
    ...
    function update_random_coupon_rules_times($inter_id,$rule_id){   //更新随机发券数
    ...
    function update_coupon_all_times($inter_id,$rule_id,$count){    //重新统计发券数
    ...
    function update_coupon_rules_times($inter_id,$rule_id){    //更新发券总数
