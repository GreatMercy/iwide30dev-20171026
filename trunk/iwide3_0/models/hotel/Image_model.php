...
class Image_model extends CI_Model {
	...
	function get_hotels_img($inter_id, $hotel_ids, $img_types, $status = 1) {
		...
		is_array ( $hotel_ids ) ? $this->db->where_in ( 'hotel_id', $hotel_ids ) : $this->db->where ( 'hotel_id', $hotel_ids );
		is_array ( $img_types ) ? $this->db->where_in ( 'type', $img_types ) : $this->db->where ( 'type', $img_types );
		is_null ( $status ) ? $this->db->where_in ( 'status', array (
				1,
				2 
		) ) : $this->db->where ( 'status', $status );
		$data = $this->db->get ( self::TAB_HOTEL_IMGS )->result_array ();
		$imgs = array ();
		foreach ( $data as $d ) {
						...
						foreach ( $d as $dd ) {
							...
							foreach ( $tmp as $t ) {
								if (! empty ( $data [0] [$type] [$t] ['param_value'] ))
									$icons [$hotel_id] [$type] [] = $data [0] [$type] [$t] ['param_value'];
							}
						}
					}
				}
