...
class Keyword_reply_model extends CI_Model{
	...
	function get_keyword_reply_text($keyword,$inter_id){
		$this->db->where(array('keyword' => $keyword,'inter_id' => $inter_id));
		$this->db->limit(1);
		...
		if($query->num_rows() > 0){
			...
			$result='';
			$this->db->select('*');
			$this->db->from('keyword k');
			//0：文本,1：图文,2：多图文,3：语音
			if($query['reply_type'] == 0 || $query['reply_type'] == 1){
				$this->db->join('replyinfo r', 'k.key_id = r.keyword_id');
				$this->db->where(array('k.keyword' => $keyword,'k.inter_id' => $inter_id));
				$this->db->limit(1);
	...
	function get_keyword_reply_text_all($keyword,$inter_id){
		...
		$rs = $this->db->query($sql,array($keyword,$inter_id));
		if($rs->num_rows() > 0){
			return $rs;
		}else{
			return null;
...
// 		}else return null;
	...
	function get_keyword_reply_text_by_id($keyword_id,$inter_id){
		$this->db->limit(1);
		$query = $this->db->get_where('keyword',array('key_id' => $keyword_id,'inter_id' => $inter_id));
		if($query->num_rows() > 0){
			$result='';
			...
			if($query['reply_type'] == 0 || $query['reply_type'] == 1){
				$this->db->join('replyinfo r', 'k.key_id = r.keyword_id');
				$this->db->where(array('k.key_id' => $keyword_id,'k.inter_id' => $inter_id));
				$this->db->limit(1);
	...
	function get_keyword_reply_list($reply_type,$inter_id,$num = null,$offset = null){
		...
		$this->db->from('keyword k');
		$this->db->join('replyinfo r', 'k.key_id = r.keyword_id');
		$this->db->where(array('k.inter_id' => $inter_id,'k.reply_type' => $reply_type, 'r.inter_id' => $inter_id));
		if($num == null){
			return $this->db->get();
		}else{
			return $this->db->get()->limit($num,$offset);
		}
	}
	...
	function get_keyword_reply_multi_list($inter_id,$num = null,$offset = null){
		...
		$this->db->from('keyword k');
		$this->db->join('replyinfo r', 'k.key_id = r.keyword_id');
		$this->db->where(array('k.inter_id' => $inter_id, 'r.inter_id' => $inter_id));
