...
class Module_func_model extends MY_Model {
     ...
     */
    public function getAccountFuncList($accountType, $roleType, $listType = 'LOGIN') {
        ...
        if (! empty ( authorityConst::$authStatesForRole [$roleType] )) {
            $roleStates = implode ( ',', authorityConst::$authStatesForRole [$roleType] );
        } else {
            return array ();
        }
        if (! empty ( authorityConst::$authRangesForAccount [$accountType] )) {
            $accountRange = implode ( ',', authorityConst::$authRangesForAccount [$accountType] );
        } else {
            return array ();
        }
        $sql = 'SELECT ' . $selects . ' FROM ' . $db->dbprefix ( authorityConst::TAB_MODULES ) . ' m
                LEFT JOIN ' . $db->dbprefix ( authorityConst::TAB_CONTROLLERS ) . ' c 
                ON c.module = m.module_code  AND c.ctlr_state in (' . $roleStates . ')  AND c.account_range in (' . $accountRange . ') 
                LEFT JOIN ' . $db->dbprefix ( authorityConst::TAB_FUNCTIONS ) . ' f 
                ON f.ctlr_id = c.ctlr_id AND f.func_state in (' . $roleStates . ') AND f.account_range in (' . $accountRange . ')
                LEFT JOIN ' . $db->dbprefix ( authorityConst::TAB_OPERATIONS ) . ' o 
                ON o.func_id = f.func_id AND o.oper_state = ' . authorityConst::OPER_STATE_VALID . ' WHERE m.module_state in (' . $roleStates . ') and m.account_range in (' . $accountRange . ')';
        ...
        foreach ( $result as $r ) {
            ...
            if (! empty ( $r ['ctlr_id'] )) {
                ...
                if (! empty ( $r ['func_id'] )) {
                    ...
                    if (! empty ( $r ['oper_id'] )) {
                        $data [$r ['module_code']] ['controllers'] [$r ['ctlr_id']] ['funcs'] [$r ['func_id']] ['operations'] [$r ['oper_id']] = array (
                                'code' => $r ['oper_code'],
                                'oper_config' => $r ['oper_config'] 
                        );
                        if ($listType == 'DISP') {
                            $data [$r ['module_code']] ['controllers'] [$r ['ctlr_id']] ['funcs'] [$r ['func_id']] ['operations'] [$r ['oper_id']] ['name'] = $r ['oper_name'];
                            $data [$r ['module_code']] ['controllers'] [$r ['ctlr_id']] ['funcs'] [$r ['func_id']] ['operations'] [$r ['oper_id']] ['des'] = $r ['oper_des'];
                        }
                    }
                }
            }
