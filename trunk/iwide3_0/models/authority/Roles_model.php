...
class Roles_model extends CI_Model {
    ...
    function check_controllers($ctlr_code,$module_code,$ctlr_id=''){
        $db = $this->_load_db ();
        if(!empty($ctlr_id))$db->where('ctlr_id !=',$ctlr_id);
        $db->where('module',$module_code);
        $db->where('ctlr_code',$ctlr_code);
        return $db->get(self::TAB_AC)->row_array();
    }

    function check_funcs($func_code,$ctlr_id,$func_id=''){
        $db = $this->_load_db ();
        if(!empty($func_id))$db->where('func_id !=',$func_id);
        $db->where('func_code',$func_code);
        $db->where('ctlr_id',$ctlr_id);
        return $db->get(self::TAB_AF)->row_array();
    }

    function check_operations($oper_code,$func_id,$oper_id=''){
        $db = $this->_load_db ();
        if(!empty($oper_id))$db->where('oper_id !=',$oper_id);
        $db->where('oper_code',$oper_code);
        $db->where('func_id',$func_id);
        return $db->get(self::TAB_AO)->row_array();
    }
...
//        if(empty($related_authorities)){
                ...
                foreach($edit_role_authorities as $m_key => $module){
                    ...
                    if(isset($module['controllers']) && !empty($module['controllers'])){
                        foreach($module['controllers'] as $c_key => $ctrl){
                            ...
                            if(isset($ctrl['funcs']) && !empty($ctrl['funcs'])){
    ...
    function update_role($params,$update){
        ...
        if(isset($update['role_authority']) && !empty($update['role_authority'])){
            $update['role_authority'] = serialize($update['role_authority']);
        }
...
//        $account_type = isset($account['admin_profile']['type'])?$account['admin_profile']['type']:3;
        ...
        $all_authorities=$this->Module_func_model->getAccountFuncList($account_type,$role_type,'DISP');//1.系统管理员，2.公众号管理员，3.普通账号
