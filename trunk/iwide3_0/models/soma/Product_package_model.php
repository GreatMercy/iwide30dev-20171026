...
// var_dump( $result, $spec_result, $result_add, $result_update, $result_delete, $gallery_add_result, $gallery_delete_result, $en_result, $combine_product_result, $extra_result);die;
    ...
    public function getComposeProductBaseInfo($filter)
    {
        ...
        $p_full_name = $this->soma_db_conn_read->dbprefix($p_tb_name);
        $c_full_name = $this->soma_db_conn_read->dbprefix($c_tb_name);

        $data = array();
        foreach($filter['where_group'] as $where)
        {
            $this->soma_db_conn_read
            ...
            {
                $this->soma_db_conn_read->where_in('p.hotel_id', $filter['hotel_id']);
            }

            if(isset($filter['name']))
            {
                $this->soma_db_conn_read->like('p.name', $filter['name']);
            }
            ...
            foreach($where as $key => $value)
            {
                if(is_array($value))
                {
                    $this->soma_db_conn_read->where_in('p.' . $key, $value);
                }
                else
                {
                    $this->soma_db_conn_read->where('p.' . $key, $value);
                }
            }

            $res = $this->soma_db_conn_read->get()->result_array();
