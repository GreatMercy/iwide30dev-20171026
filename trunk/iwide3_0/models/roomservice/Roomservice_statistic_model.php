...
class Roomservice_Statistic_Model extends MY_Model{
    ...
    public function get_sum_statistic($filter = array()){//sum(if(pay_status=1&&pay_way!=3 || pay_way=3&&order_status<=20,1,0)) as all_success_orders_count,
        $sql = "select inter_id,count(distinct(openid)) as all_mem_count,count(*) as all_orders_count,sum(if(pay_way = 3,sub_total,pay_money)) as income_money,sum(if(order_status=0,1,0)) as wait_accept,sum(if(order_status=5,1,0)) as wait_send,sum(if(order_status=10,1,0)) as sending,sum(if(order_status=20,1,0)) as finish,sum(if(order_status=25 ||order_status=26||order_status=27 ,1,0)) as cancel from iwide_roomservice_orders where 1=1";
        //获取房间数
        ...
        //成功人数 成功订单数
        $succ_sql = "select inter_id,count(distinct(openid)) as success_mem_count,count(*) as success_order_count from iwide_roomservice_orders where (pay_way=3 && order_status <=20 || pay_way!=3 && order_status <=20 && pay_status = 1) ";
