...
WHERE ge.distribute=? AND ga.inter_id=? AND ga.saler>0 ";
        $argvs [] = $params['inter_id'];
...
// 		return $query;
	...
	public function get_mall_grades_order($inter_id, $arvg = array(), $limit = NULL, $offset = 0) {
        $sql = "SELECT s.order_id,s.sub_order_id,s.pms_order_id,s.member_card_no,s.membership_number,s.customer,s.cellphone,s.product,s.product_group,s.order_time,
            ...
            WHERE s.inter_id=? AND NOT ISNULL(hs.qrcode_id) AND a.saler<>0";
		$params [] = $inter_id;
	...
	public function get_mall_grades_order_count($inter_id, $arvg = array()) {
		$sql = "SELECT COUNT(s.order_id) nums FROM iwide_mall_order_summary s LEFT JOIN iwide_distribute_grade_all a ON s.order_id=a.grade_id AND s.inter_id=a.inter_id LEFT JOIN iwide_hotel_staff hs ON a.inter_id=hs.inter_id AND a.saler=hs.qrcode_id WHERE s.inter_id=? AND NOT ISNULL(hs.qrcode_id)";
		$params [] = $inter_id;
		if (! empty ( $arvg['order_id'] )) {
			$sql .= " AND s.order_id LIKE ?";
			$params [] = "%{$arvg['order_id']}%";
