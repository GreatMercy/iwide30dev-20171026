...
class Hotel_base {
	...
	public function add_essential_param($param = array()) {
		if (WEB_AREA == 'front') {
			...
			$ori_f_saler = $this->CI->input->get ( 'ofsaler', true );
			if ($ori_f_saler) {
				$param ['ofsaler'] = $ori_f_saler;
			}
			$link_saler = $this->CI->input->get ( 'lsaler', true );
			$link_f_saler = $this->CI->input->get ( 'lfsaler', true );
			...
			} elseif (! empty ( self::$_basic_param ['own_f_saler'] )) {
				$param ['lfsaler'] = self::$_basic_param ['own_f_saler'];
				if ($link_f_saler && $link_f_saler != $param ['lfsaler']) {
					$param ['ofsaler'] = $link_f_saler;
					self::$_basic_param ['saler_redirect'] = 1;
				}
				if (empty ( $link_f_saler )) {
					self::$_basic_param ['saler_redirect'] = 1;
				}
			} else {
				if ($link_saler) {
