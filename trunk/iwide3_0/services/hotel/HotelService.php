...
// 		) );
	...
	function to_comment() {
		$data = $this->common_data;
		$oid = intval ( $this->getCI()->input->get ( 'oid' ) );
		$this->getCI()->load->model ( 'hotel/Order_model' );
		$list = $this->getCI()->Order_model->get_main_order ( $this->getCI()->inter_id, array (
			'oid' => $oid,
			'openid' => $this->getCI()->openid,
			'member_no' => $this->member_no,
			'idetail' => array (
...
//$this->db->insert('weixin_text',array('content'=>'cancel_main_order+'.json_encode($info),'edit_date'=>date('Y-m-d H:i:s')));
	...
	function new_comment_sub() {    //提交评论
        ...
        if($res && isset($res['status']) && $res['status']==1){   //更新评论统计信息
            $this->getCI()->Comment_model->update_hotel_score_from_redis($this->getCI()->inter_id,$data ['hotel_id'],$data,'add',$this->getCI()->openid);
