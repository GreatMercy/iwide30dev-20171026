 ...
 * Time: 12:08
 */
class Wangxin extends MY_Controller{
	private $url = 'http://106.75.7.248:8090/ipmsgroup/CRS';
	private $usr_url = 'http://106.75.7.246:8090/ipmsmember/membercard/';
//	private $url='http://106.75.10.41:8100/ipmsgroup/CRS';
//	private $usr_url = 'http://106.75.10.41:8100/ipmsmember/membercard/';
	private $sales_channel='WBC';
	private $price_sales_channel='CRS';
	private $hotel_groupid=2;
	private $src='CRS';
	private $pms_auth;

//	private $inter_id='a487242633'; //开发
//	private $inter_id='a487647571'; //测试
	private $inter_id='a487576098';//生产
	...
	public function __construct(){
		...
		$this->pms_auth = array(
			...
			'src'          => $this->src,
			'priceSalesChannel'=>$this->price_sales_channel,
		);
...
//		$url='http://106.75.7.248:8090/ipmsgroup/CRS/hotels?hotelGroupCode=LILIAN';
		...
		if(!empty($res['result'])){
			...
			if($hotels){
				ksort($hotels);
				file_put_contents(FD_PUBLIC.'/tmp_data/wangxin_hotels_prod.json',json_encode($hotels));
			}
	...
	public function catches(){
		$json=file_get_contents(FD_PUBLIC.'/tmp_data/wangxin_hotels_pms.json');
	...
	public function prod_catches(){
		$json=file_get_contents(FD_PUBLIC.'/tmp_data/wangxin_hotels_prod.json');
