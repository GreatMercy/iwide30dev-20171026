...
// 		@Editor lGh 支付回调签名验证
	...
	function hotel_debt() {
        ...
        $data ['type'] = 'weixin';
        $this->db->insert ( 'pay_log', $data );
        MYLOG::w ( json_encode ( $data ), 'hotel' . DS . 'pay_payreturn' . DS . 'debt' );
        $this->load->model ( 'Pay/Wxpay_model' );
        $check = $this->Wxpay_model->wxpay_return_sign ( $data ['inter_id'], $arr );
        if (! $check) {
            echo 'error';
...
//        $result = (array) simplexml_load_string( $xml, 'SimpleXMLElement', LIBXML_NOCDATA );
        ...
        if($update_result['err']>0){
            ...
            //加入分销数据
            if( isset( $deposit_data['distribution_money']) && $deposit_data['distribution_money']>0 && isset($order_info['data']['distribution_num']) && $order_info['data']['distribution_num'] > 0){
