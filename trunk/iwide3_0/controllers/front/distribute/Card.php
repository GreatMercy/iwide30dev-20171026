<?php 
class Card extends MY_Front{
	function index(){
		$package = $this->getSignPackage();
		$tmp['cardId']='pX3WojiTPhgNcwupKYt0qCYgJaQI';
		...
		$str = createNonceStr();
		$p=$this->getSignCard($tmp['cardId'],$apiTicket,$code,$str);
		...
		wx.config({
			...
			jsApiList: [ 'checkJsApi', 'addCard', 'chooseCard', 'openCard' ]
		});
		wx.ready(function () {
		  wx.addCard({
			      cardList: [
			      ...
			      success: function (res) {alert('已添加卡券：' + JSON.stringify(res.cardList));},
			      cancel: function (res) {alert(JSON.stringify(res))}
			    });
		    });
			</script>
			</html>
