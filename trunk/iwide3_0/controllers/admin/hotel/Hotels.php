...
class Hotels extends MY_Admin {
	...
	public function edit() {
		...
		if ($id) {
			...
			$hotel=$this->Hotel_model->get_hotel_detail($this->session->get_admin_inter_id (),$id);
			if ($hotel){
...
// 		$hotel_ser = $this->db->get ( 'hotel_images' )->result_array ();
	...
	public function edit_post() {
        ...
        if(isset($post['province']) && $post['province']=='省份选择')$post['province']=NULL;
        if(isset($post['city']) && $post['city']=='地市选择')$post['city']=NULL;
        if(isset($post['area']) && $post['area']=='区县选择')$post['area']=NULL;
