...
// 	protected $label_module= NAV_HOTELS;
	...
	function __construct(){
		parent::__construct();
        $this->load->helper('appointment');//加载函数
	}
	...
	public function index()
    {
        ...
        $filter['group_name'] = $params['group_name'] ? addslashes($params['group_name']) : '';

        $per_page = 20;
        $cur_page = $params['page'] ? $params['page'] : 1;
        ...
        //查询总记录数
        $total = $this->distribute_group_model->distribute_group_count($filter);

        //分页
        $arr_page = get_page($total, $cur_page, $per_page);

        //查询数据列表
        $data = $this->distribute_group_model->distribute_group_list($filter,'*',$cur_page,$per_page);
        ...
        $url = site_url('/distribute/distribute_group/index?'.$http_build_query);
	...
	public function group_add(){
		...
		if($submit){
            ...
            if((empty($filter['check_count']) || $filter['check_count'] < 0) && ($filter['type'] == 2)){
     ...
     * 查询公众号/酒店部门接口
     */
    public function hotel_department()
    {
        $post = $this->input->post();
        $inter_id= $this->session->get_admin_inter_id();
        $hotel_id = !empty($post['hotel_id']) ? addslashes($post['hotel_id']) : '';
        $this->load->model('distribute/distribute_group_model');
        $data = $this->distribute_group_model->get_department($inter_id,$hotel_id);
    ...
    public function hotel_staff()
    {
        ...
        $inter_id= $this->session->get_admin_inter_id();
        $hotel_id = !empty($post['hotel_id']) ? addslashes($post['hotel_id']) : '';
        $master_dept = !empty($post['master_dept']) ? addslashes($post['master_dept']) : '';
        $group = array(
            'hotel_ids' => $hotel_id,
            'department_ids' => $master_dept,
        );
        $this->load->model('distribute/distribute_group_model');
        $data = $this->distribute_group_model->get_salers_info_list($inter_id,$group);
        ajax_return(1,'成功',$data);
    }
