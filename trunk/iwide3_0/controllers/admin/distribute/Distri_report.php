...
// 			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 12, 1, '支付状态' );
	...
	// 			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 2, $row, $item['istatus'] );
				...
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 5, $row, $hotels[$item['hotel_id']] );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 6, $row, date('Y-m-d H:i:s',$item['order_time']) );
...
// 				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 12, $row, $paid[$item['paid']] );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 13, $row, isset($hotels[$exts[$item['sid']]['hotel_id']]) ? $hotels[$exts[$item['sid']]['hotel_id']] : '-' );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 14, $row, isset($exts[$item['sid']]['staff_name']) ? $exts[$item['sid']]['staff_name'] : '-' );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 15, $row, isset($exts[$item['sid']]['grade_total']) ? $exts[$item['sid']]['grade_total'] : '-' );
				$row ++;
			}
		}
		if($type == 'staffs'){
			...
			foreach ( $res as $item ) {
				...
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 0, $row, isset($exts[$item['sid']]['staff_name']) ? $exts[$item['sid']]['staff_name'] : '-' );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 1, $row, isset($exts[$item['sid']]['saler']) ? $exts[$item['sid']]['saler'] : '-' );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 2, $row, isset($exts[$item['sid']]['send_time']) ? $exts[$item['sid']]['send_time'] : '-' );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 3, $row, isset($exts[$item['sid']]['grade_total']) ? $exts[$item['sid']]['grade_total'] : '-' );
	// 			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 2, $row, $item['istatus'] );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 4, $row, isset($hotels[$exts[$item['sid']]['hotel_id']]) ? $hotels[$exts[$item['sid']]['hotel_id']] : '-' );
				...
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 7, $row, $hotels[$item['hotel_id']] );
				...
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 15, $row, isset($exts[$item['sid']]['grade_total']) ? $exts[$item['sid']]['grade_total'] : '-' );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 16, $row, isset($hotels[$exts[$item['sid']]['hotel_id']]) ? $hotels[$exts[$item['sid']]['hotel_id']] : '-' );
		...
		if($type == 'hotels'){
			...
			$row = 2;
			foreach ( $res as $item ) {
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 0, $row, isset($hotels[$exts[$item['sid']]['hotel_id']]) ? $hotels[$exts[$item['sid']]['hotel_id']] : '-' );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 1, $row, isset($exts[$item['sid']]['send_time']) ? $exts[$item['sid']]['send_time'] : '-'  );
				$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 2, $row, isset($exts[$item['sid']]['grade_total']) ? $exts[$item['sid']]['grade_total'] : '-' );
	// 			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 2, $row, $item['istatus'] );
...
// 		$config['suffix']            = $sub_fix;
	...
	public function ext_fans_room_order2(){
		...
		foreach ($res as $item ) {
				...
				$data = $data.$item['saler'].",";
 				$data = $data.mb_convert_encoding($item['master_dept'],'gb2312','utf-8').",";
				$data = $data.mb_convert_encoding($item['hotel_name'],'gb2312','utf-8').",";
	...
	public function exnew_fas_hotel(){
		...
		if(empty($avgs['begin_time'])){
			$avgs['begin_time'] = date("Y-m-d H:i:s",(time()-2592000));
		}
		if(empty($avgs['end_time'])){
			$avgs['end_time'] = date("Y-m-d H:i:s",time());
		}
...
// 		$filterH ['hotel_id'] = $avgs['hotel_id'];
	...
	public function ext_exnew_add_fans(){
		...
		foreach ($res as $item ) {
			...
			$data = $data.$item['saler']." ,";
			$data = $data.iconv('utf-8','gb2312',$item['name']).",";
			$data = $data.iconv('utf-8','gb2312',$item['hotelname']).",";
	...
	public function ext_saler_grades(){
		...
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 7, 1, '旧商城绩效金额' );
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 8, 1, '新商城绩效金额' );
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 9, 1, '套票商城绩效金额' );
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 10, 1, '拼团商城绩效金额' );
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 11, 1, '会员卡绩效金额' );
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 12, 1, '未发绩效' );
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 13, 1, '已发绩效' );
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 14, 1, '绩效金额排行榜' );
		// Fetching the table data
		$row = 2;
		foreach ( $res as $item ) {
			...
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 7, $row, $item->GRADE_SHOPS );
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 8, $row, $item->GRADE_MALL );
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 9, $row, $item->GRADE_PACKAGE );
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 10, $row, $item->GRADE_GROUPON );
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 11, $row, $item->GRADE_MEMBER );
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 12, $row, $item->UNDELIVER );
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 13, $row, $item->GRADE_TOTAL-$item->UNDELIVER );
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 14, $row, $item->rank );
			$row ++;
		}
		$objPHPExcel->setActiveSheetIndex ( 0 );
		$objWriter = IOFactory::createWriter ( $objPHPExcel, 'Excel5' );
	...
	public function ext_mall_orders(){
        ...
        if(!empty($keys[9])){
            $params['department'] = urldecode($keys[9]);
        }
		$this->load->model ( 'hotel/hotel_model' );
		$filterH ['inter_id'] = $admin_profile['inter_id'];
		$hotels = $this->hotel_model->get_hotel_hash ( $filterH );
