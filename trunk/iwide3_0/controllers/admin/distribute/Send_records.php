...
// 		}
		...
		$config ['base_url'] = base_url ( "index.php/distribute/send_records/records/".$batch_no.'_'.$from.'_'.$to );
		$config ['total_rows'] = $this->send_records_model->get_parent_records ( $inter_id, $batch_no, $from, $to)->num_rows();
		...
		$query = $this->send_records_model->get_parent_records ( $inter_id, $batch_no, $from, $to, $config ['per_page'],$page )->result();
		$view_params = array (
	...
	public function sub_records(){
		...
		$config['suffix'] = '?bn='.$batch_no;
		$config ['cur_tag_open'] = '<a class="number current" href="#">';
		$config ['cur_tag_close'] = '</a>';
		$config ['base_url'] = site_url("distribute/send_records/sub_records/".$order_no.'_'.$saler_no.'_'.$saler_name.'_'.$msg_typ.'_'.$batch_no);
		$config ['total_rows'] = $this->send_records_model->get_records ( $inter_id,$batch_no,$msg_typ,$order_no,$saler_no,$saler_name)->num_rows();
		$config ['cur_tag_open'] = '<li class="paginate_button active"><a>';
		$config ['cur_tag_close'] = '</a></li>';
		$config ['num_tag_open'] = '<li class="paginate_button">';
		$config ['num_tag_close'] = '</li>';
		...
		$config ['next_tag_close'] = '</li>';
		$this->pagination->initialize ( $config );
		$query = $this->send_records_model->get_records ( $inter_id,$batch_no,$msg_typ,$order_no,$saler_no,$saler_name, $config ['per_page'],$page )->result();
	...
	public function ext_records(){
		...
		$query = $this->send_records_model->get_parent_records ( $inter_id, $batch_no, $from, $to)->result_array();
	...
	public function ext_sub_records(){
		...
		$query = $this->send_records_model->get_records ( $inter_id,$batch_no,$msg_typ,$order_no,$saler_no,$saler_name )->result_array();
		$this->load->model('hotel/hotel_model');
		...
		$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 11, 1, '备注' );
		...
		foreach ( $query as $item ) {
			...
			$objPHPExcel->getActiveSheet ()->setCellValueByColumnAndRow ( 11, $row, $item['status'] == 1 ? '--' : $item['remark'] );
			$row ++;
		}
		$objPHPExcel->setActiveSheetIndex ( 0 );
		$objWriter = IOFactory::createWriter ( $objPHPExcel, 'Excel5' );
	...
	public function batch_log() {
		...
		$config ['total_rows'] = $this->send_records_model->get_batch_logs ( $inter_id, $batch_no, $from, $to)->num_rows();
		$config ['cur_tag_open'] = '<li class="paginate_button active"><a>';
		$config ['cur_tag_close'] = '</a></li>';
		$config ['num_tag_open'] = '<li class="paginate_button">';
		$config ['num_tag_close'] = '</li>';
		...
		$config ['next_tag_close'] = '</li>';
		$this->pagination->initialize ( $config );
		$query = $this->send_records_model->get_batch_logs ( $inter_id, $batch_no, $from, $to, $config ['per_page'],$page )->result();
		$view_params = array (
				'pagination' => $this->pagination->create_links (),
				'res' => $query,
				'order_no' => $batch_no,
	...
	public function saler_logs(){
		...
		$config ['total_rows'] = $this->send_records_model->get_salers_log ( $inter_id,$batch_no,$msg_typ,$saler_no,$saler_name)->num_rows();
		$config ['cur_tag_open'] = '<li class="paginate_button active"><a>';
		$config ['cur_tag_close'] = '</a></li>';
		$config ['num_tag_open'] = '<li class="paginate_button">';
		$config ['num_tag_close'] = '</li>';
		...
		$config ['next_tag_close'] = '</li>';
		$this->pagination->initialize ( $config );
		$query = $this->send_records_model->get_salers_log ( $inter_id,$batch_no,$msg_typ,$saler_no,$saler_name, $config ['per_page'],$page )->result();
		$this->load->model('hotel/hotel_model');
	...
	public function record_orders(){
		...
		$config ['total_rows'] = $this->send_records_model->get_partner_logs ( $inter_id,$record_id)->num_rows();
		$config ['cur_tag_open'] = '<li class="paginate_button active"><a>';
		$config ['cur_tag_close'] = '</a></li>';
		$config ['num_tag_open'] = '<li class="paginate_button">';
		$config ['num_tag_close'] = '</li>';
		...
		$config ['next_tag_close'] = '</li>';
		$this->pagination->initialize ( $config );
		$query = $this->send_records_model->get_partner_logs ( $inter_id,$record_id, $config ['per_page'],$page )->result();
		$this->load->model('hotel/hotel_model');
	...
	public function ext_batch_log() {
		...
		$query = $this->send_records_model->get_batch_logs ( $inter_id, $batch_no, $from, $to)->result_array();
		$this->load->library ( 'PHPExcel' );
		$this->load->library ( 'PHPExcel/IOFactory' );
		$objPHPExcel = new PHPExcel ();
		$objPHPExcel->getProperties ()->setTitle ( "export" )->setDescription ( "none" );
	...
	public function ext_saler_logs(){
		...
		$query = $this->send_records_model->get_salers_log ( $inter_id,$batch_no,$msg_typ,$saler_no,$saler_name )->result_array();
		$this->load->library ( 'PHPExcel' );
		$this->load->library ( 'PHPExcel/IOFactory' );
		$objPHPExcel = new PHPExcel ();
		$objPHPExcel->getProperties ()->setTitle ( "export" )->setDescription ( "none" );
