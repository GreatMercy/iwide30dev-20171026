...
// 			}
			...
			if (!empty($list)){
				$list=$list [0];
				$use_condition=json_decode($list['use_condition'],TRUE);
				if (isset($use_condition['member_level'])){
					$data ['use_condition'] ['member_level']=$use_condition['member_level'];
				}
				if (isset($use_condition['member_levels'])){
					$data ['use_condition'] ['member_levels']=$use_condition['member_levels'];
				}
			}
		...
		redirect ( site_url ( 'hotel/prices/index' ) . '?hotel=' . $data ['hotel_id'] . '&r=' . $data ['room_id'] );
	}
	...
	public function edit_code_post() {
		...
		if (isset($member_level)&&$member_level != - 1)
			$data ['use_condition'] ['member_level'] = intval ( $member_level );
		
		if (!empty($member_levels)){
			$data ['use_condition'] ['member_levels'] = $member_levels;
		}
		...
		if (strtotime ( $book_time_s ) && strtotime ( $book_time_e ) && $book_time_s < $book_time_e) {
			$data ['use_condition'] ['book_time'] ['s'] = $book_time_s;
			$data ['use_condition'] ['book_time'] ['e'] = $book_time_e;
		}
		$max_minute = intval ( $this->input->post ( 'min_min' ) );
		if (! empty ( $max_minute )) {
			$data ['use_condition'] ['min_min'] = $max_minute;
		}
		$pre_day = $this->input->post ( 'pre_day' );
		if ($pre_day != '' && is_numeric ( $pre_day ))
			$data ['use_condition'] ['pre_d'] = intval ( $pre_day );
