...
class Bonus extends MY_Admin {
    ...
    public function gr_edit() {
        ...
        if (! empty ( $rule_id )) {
            ...
            if(!empty($rule['give_rule'])){
                ...
                if(isset($give_rule->all)){
                    $rule['give_type']='all';
                    $rule['allCost']=$give_rule->all->cost;
                    $rule['allAmount']=$give_rule->all->amount;
                }else{
                    $rule['give_type']='part';
                    foreach($give_rule as $levels=>$arr){
    ...
    public function gr_edit_post(){
        ...
        if(isset($data['member_level'])&&$data['member_level']=='all'){
            $give_rule['all']=array('cost'=>$data['allcost'],'amount'=>$data['allamount']);
        }else{
            if(isset($data['levels'])){
                foreach($data['levels'] as $arr){
                    $give_rule[$arr]=array('cost'=>$data['cost'][$arr],'amount'=>$data['amount'][$arr]);
                }
            }
        }
