	...
	public function edit_post() {
		...
		if(!$inter_id || $inter_id == FULL_ACCESS) {
	    	...
	    	if(isset($post['bg_img'])) {
				...
				if(isset($post['bg_img_url']) && $post['bg_img_url'] != '') {
					$post['bg_img'] = $post['bg_img_url'];
				}
				if(isset($post['bg_img_url'])){
					unset($post['bg_img_url']);
				}
			}
			if(isset($post['btn_img'])) {
				$post= $this->_do_upload($post, 'btn_img');
			} else {
				if(isset($post['btn_img_url']) && $post['btn_img_url'] != '') {
					$post['btn_img'] = $post['btn_img_url'];
				}
				if(isset($post['btn_img_url'])){
					unset($post['btn_img_url']);
				}
			}
